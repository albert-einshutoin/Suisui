...
- 外部 SaaS なしでも、Suisui 単体で task board + review execution の価値があるか。
...
- [x] `SuisuiApp` 起動時に SQLite、Keychain、audit logger、ToolRegistry を 1 つの app container から生成する。
...
- [x] report はblocking gateが残る間は `NOT READY` とexit 2を返し、product research advisoryだけが残る場合は`READY WITH ADVISORIES`とexit 0を返す。
- [x] `script/check_automated_release_preflight.sh` で CI、SQLite CRUD、runtime accessible CRUD、layout stability、Xcode build、visible-window launch、seeded VoiceOver review candidate 上の runtime AX、MCP compliance を一括検証できる。
- [x] `./script/check_automated_release_preflight.sh` の標準実行で `.tmp/automated-release-preflight-$(git rev-parse --short HEAD).md` を書き出し、自動proof gateのpass結果をレビュー可能なMarkdown証跡として保存できる。
...
- [x] VoiceOver手動証跡は`Source commit`を記録し、`Status: passed`の場合は現在のrelease-candidate product source commitと一致しない証跡をrelease blockerにする。competitor hands-onの同じ不整合はProduct Research advisoryとして表示する。
- [x] VoiceOver / competitor hands-onの`Status: passed`手動証跡は`Generated by: script/create_*_evidence.sh`を含む必要があり、手書きのVoiceOver passed証跡はrelease blocker、competitor passed証跡はadvisory incompleteとして扱う。
- [x] competitor benchmarkの`Source commit`が`Status: passed`のcompetitor hands-on証跡と同じrelease候補commitでない場合は、Product Research advisoryとして表示する。
- [x] UI screenshot証跡は `Sources/SuisuiApp` / `Sources/SuisuiCore` / `Package.swift` の最新UI source commitを記録し、UI/runtime source変更後に古いスクリーンショットをrelease evidenceとして再利用しない。
- [x] `release_readiness_report.sh` は CI、SQLite CRUD、runtime accessible CRUD、layout stability、Xcode build、visible-window launch、runtime AX をskipしたままrelease readyにしない。
- [x] `SUISUI_AUTOMATED_PROOF_GATES=1 ./script/release_readiness_report.sh` で自動proof gateをまとめてfinal report内に含められる。
...
- [x] case-insensitive macOS filesystem 上で `Suisui` app product と CLI product が同じ build output を上書きしないことを regression test で確認する。
- [x] CLI product を `suisui-cli` にし、`Suisui` GUI app binary と明確に分離する。
- [x] `suisui-cli --help` が usage を出して exit 0 になる。
...
- [x] `suisui-cli status` は app default SQLite DB を read-only で開き、active / archived project、open / due task、Knowledge Frame 数を実データから出す。
- [x] `suisui-cli tasks due` は completed task と archived project 配下 task を除外し、期限到来 task だけを表示する。
- [x] `suisui-cli frames search <query>` は `SQLiteKnowledgeFrameStore.search` を使い、FTS の実検索結果を表示する。
- [x] app DB が無い場合は SQLite ファイルを作らず `database: missing` を返す。
- [x] GUI と CLI は `SuisuiAppDatabaseLocation` で同じ app default DB path を共有し、path drift を防ぐ。
- [x] release readiness report の runtime scan は `Sources/SuisuiCLI` と `skeleton` / `placeholder` marker も対象にする。
...
- [x] 開発機の packaging smoke は `SUISUI_REQUIRE_SIGNED_PACKAGE=0` と `SUISUI_REQUIRE_NOTARIZED_PACKAGE=0` の両方を明示した場合だけ通せる。
...
- [x] `verify_release_environment.sh` は `dist/Suisui.app/Contents/Info.plist` の `CFBundleIdentifier` / `CFBundleShortVersionString` / `CFBundleVersion` を `packaging/app_metadata.env` と照合する。
...
- [x] `sign_app.sh` は `SUISUI_SIGNING_IDENTITY` が `Developer ID Application:` で始まらない場合、codesign 実行前に失敗する。
...
- [x] 複数 package artifact を作った場合は `SUISUI_RELEASE_ARTIFACT_SHA256_FILE` で証跡対象を明示する。
...
- [x] `verify_release_environment.sh` は signed app から `codesign -d --entitlements :-` で entitlements を読み、`packaging/Suisui.entitlements` と照合する。
...
- [x] `SUISUI_RELEASE_PREFLIGHT_ONLINE=1` の場合は `xcrun notarytool history --keychain-profile` で configured notary profile を検証する。
...
- [x] `create_release_evidence.sh` は `SUISUI_SIGNING_IDENTITY` / `SUISUI_NOTARY_PROFILE` がない状態で成功証跡を作らない。
...
- [x] `generate_appcast.sh` は release mode で `SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX` を必須にし、HTTPS 以外、予約ドメイン、ローカルドメインを拒否する。
...
- [x] `verify_appcast.sh` は configured `SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX` と enclosure URL が一致しない appcast を拒否する。
...
- [x] release checklist は generated `dist/releases/appcast.xml` を `SUISUI_REQUIRE_RELEASE_APPCAST=1` で検証する。
...
- [x] release checklist は user download 用 DMG と Sparkle appcast 用 ZIP を `SUISUI_PACKAGE_FORMAT=all` で同時生成する。
- [x] release evidence / final preflight は `SUISUI_RELEASE_ARTIFACT_SHA256_FILE` で DMG checksum を明示し、複数 checksum の自動選択を避ける。
...
- [x] release build は `SUISUI_SPARKLE_FEED_URL` と `SUISUI_SPARKLE_PUBLIC_ED_KEY` が揃わない場合に Swift build 前に失敗する。
...
- [x] Google / Gmail / Slack / Drive / Notion connector 実装を `SuisuiCore` から分離し、`SuisuiExternalConnectors` target に置く。
- [x] `Suisui` app target と `SuisuiCLI` target は `SuisuiExternalConnectors` に依存しない。
...
- [x] テスト: `Sources/SuisuiCore` に external SaaS connector symbols が残らないことを確認する。
...
- [x] `suisui-cli status` の SQLite `COUNT(*)` decode は欠落 / 不正値を `0` に丸めない。
- [x] `SuisuiCLIReadOnlyReporter.parseCountValue` は missing count と invalid count を `LocalStoreDecodingError` として返す。
...
- [x] Scene-level `.preferredColorScheme(effectiveAppearancePreference.colorScheme)` と Settings の同じ `@AppStorage` key を使い、通常画面ごとにテーマ状態が分岐しない。UI evidence capture時だけ `SUISUI_APPEARANCE_PREFERENCE` でLight/Dark/Systemを明示できる。
...
- [x] Board card の drag payload は plain text `String` ではなく、Suisui task 専用の `Transferable` payload にする。
...
- [x] 完了条件: 他アプリやテキスト入力由来の偶然の文字列 drop と、Suisui task card drag operation を UI / code 上で分離する。
...
- [x] `Sources/SuisuiCore` / `Sources/SuisuiApp` / `Sources/SuisuiCLI` の固定 scan path が壊れたら `READY` を出さない。
- [x] `Sources/SuisuiExternalConnectors` は app / CLI runtime 依存から外したまま、存在する場合は release source scan に含め、OSS提供target内の mock / fixture marker も release blocker にする。
...
- [x] `SuisuiAppDelegate` は通常起動時に regular app として activate し、ウィンドウが見えていない場合は `WindowGroup` の New Window action を送る。
...
- [x] `release_readiness_report.sh` は `SUISUI_RELEASE_ACTIONS_FILE` 指定時に残blockerのoperator action summaryを書き出す。
...
- [x] action summary は report生成commit と、手動VoiceOver / competitor hands-on証跡が一致すべき release-candidate product source commit を別々に表示する。
- [x] action summary は今回の実行で発生した具体blockerを `Current Blocker Groups` のチェックリストとして列挙する。
- [x] action summaryは`Operator Priority Queue`を`Current Blocker Groups`より前に出し、手動VoiceOverとrelease-machineが減らせるblocker数、競合hands-onのadvisory数、release-machine内の環境blocker件数、Phase routing対象の手動項目数、worksheet -> generated commandの順序を示す。
- [x] action summaryは`Blocker Buckets`でAutomated Proof Gates / Manual VoiceOver / Release Machine / Phase Checklist / Otherを分類し、競合hands-onは`Current Advisories`と専用sectionへ分離する。
...
- [x] `check_automated_release_preflight.sh` は通過後に VoiceOver / competitor helper を release-candidate product source commit、release-machine helper を current release evidence source commit で再生成し、helper freshness を片寄らせない。
...
- [x] action summaryは`Manual VoiceOver Blockers`と`Competitor Hands-On Advisories`に手動証跡の不足項目を分離表示し、競合調査を完了扱いにせず、配布blockerにも数えない。
...
- [x] Generated VoiceOver evidence command reads reviewer/environment/focus observations from the completed worksheet, avoiding a second command-line placeholder copy step for VoiceOver evidence.
- [x] action summary は VoiceOver / competitor hands-on の current `Source commit` に対応する pending evidence path も併記する。
- [x] action summary は VoiceOver / competitor hands-on の証跡生成コマンドを必須フラグ込みで表示し、VoiceOver は worksheet 入力、competitor hands-on は placeholder 置換で実測値を渡すことを明記する。
...
- [x] `.tmp/release-machine/create-release-evidence-command.sh` runs `SUISUI_RELEASE_PREFLIGHT_ONLINE=1 ./script/verify_release_environment.sh` after writing release evidence so release-machine operators see the final gate before rerunning the readiness report.
...
- [x] release readiness は `docs/release/manual-unblockers.md` から manual-only evidence boundary が消えた場合 blocker にする。
...
- [x] `script/prepare_release_manual_helpers.sh` は release-candidate product source commit の VoiceOver pending preview / launch env / worksheet / command、competitor pending evidence、competitor benchmark pending worksheet、competitor worksheet / command と、current release evidence source commit の release-machine worksheet / command を一括再生成し、passed evidence を書かない。
...
- [x] action summary は古い `.tmp/voiceover-review/*-pending-<old-commit>.md` / `.tmp/competitor-hands-on/*-pending-<old-commit>.md` と legacy default preview `.tmp/competitor-hands-on/evidence.md` を ignored stale preview として表示し、operatorが別release候補のcontextをtracked evidenceへ転記しないようにする。
- [x] `script/prepare_release_manual_helpers.sh --prune-stale` は release-candidate source commit のhelper再生成後、古いpending previewとlegacy default previewを削除し、passed evidenceを書かない。
...
- [x] `scripts/ci.sh` は `SUISUI_CI_TMP_ROOT` / `SUISUI_CI_TMPDIR` を使って SwiftPM の `TMPDIR` を release preflight 専用領域へ隔離する。
- [x] `scripts/ci.sh` は自分で作った `suisui-ci-tmp.*` を `EXIT INT TERM` trap で削除し、`swift test` が作る `TemporaryDirectory.*` を `.tmp` 直下に積み上げない。
...
- Runtime composition は `Sources/SuisuiApp/SuisuiApp.swift` の `AppPreviewFactory` を廃止または改名して、preview ではなく production app container として扱う。
- SQLite path は `~/Library/Application Support/Suisui/Suisui.sqlite` を primary にする。
...
- `swift test`
- `xcodebuild -workspace .swiftpm/xcode/package.xcworkspace -scheme Suisui -destination 'platform=macOS' build`
- `./scripts/verify.sh`
- `./script/build_and_run.sh --verify`
- `swift build --product suisui-cli && .build/debug/suisui-cli --help`
- `SUISUI_RELEASE_PREFLIGHT_ONLINE=1 ./script/verify_release_environment.sh` (Developer ID / notary / `packaging/release-evidence.json` が揃った release machine で green にする。開発機では blocker 出力を確認する。)
