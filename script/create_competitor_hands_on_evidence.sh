...
write_hands_on_worksheet() {
...

  {
    ...
    printf '%s\n' '3. Remove `To Fill`, unchecked `[ ]`, and instructional placeholder text before running the generated command.'
    printf '%s\n' '4. Replace every placeholder in the generated passed command with observations from this worksheet.'
    printf '%s\n' '5. Run the generated command only after the hands-on pass is complete.'
    printf '%s\n' '6. Rerun `./script/release_readiness_report.sh` and confirm the competitor hands-on section is green.'
  } >"$WORKSHEET_FILE"
}
...
write_pending_benchmark() {
...

  {
    ...
    printf '%s\n' '## Closeout'
    printf '\n'
    printf '%s\n' '1. Change `Status: pending` to `Status: completed` after filling the benchmark findings from the hands-on pass.'
    printf '%s\n' '2. Fill Notion, Todoist, Linear, Motion, Ship, Defer, and Reject with concrete observations.'
    printf '%s\n' '3. Replace this pending worksheet with `script/create_competitor_hands_on_evidence.sh --passed --benchmark-output ...` output after the hands-on pass.'
    printf '%s\n' '4. Do not copy this file into `docs/product/competitor-benchmark.md` as passed evidence.'
    printf '%s\n' '5. Rerun `./script/release_readiness_report.sh`; pending, unchecked, or worksheet language must remain a blocker.'
  } >"$BENCHMARK_FILE"
}
...
write_competitor_evidence_command() {
...

  {
    ...
    printf '%s\n' '  fi'
    printf '%s\n' ''
    printf '%s\n' '  if grep -Eq "(^Status:[[:space:]]*pending$|To Fill|This worksheet is not release evidence|Replace every placeholder|not release evidence)" "$COMPETITOR_WORKSHEET_FILE"; then'
    printf '%s\n' '    printf "BLOCKER: competitor hands-on worksheet is missing, stale, or incomplete: remove pending/template instructions before writing evidence.\n" >&2'
    printf '%s\n' '    exit 2'
    printf '%s\n' '  fi'
    printf '%s\n' ''
    ...
    printf '%s\n' '  fi'
    printf '%s\n' ''
    printf '%s\n' '  for required_label in "Reviewer" "Review date" "macOS version" "Browser / desktop app versions" "Account tiers / paid trial details" "Elapsed hands-on time with per-competitor timing" "Screenshot or note locations kept outside release evidence" "Setup steps before first useful task" "Clicks / keystrokes for capture and status movement" "Inspector/detail clarity for repeated solo PM work" "Automation or recommendation trust issues" "Ship" "Defer" "Reject"; do'
    ...
    printf '%s\n' '}'
    printf '\n'
    printf 'COMPETITOR_BENCHMARK_WORKSHEET_FILE=%q\n' "$BENCHMARK_FILE"
    printf '%s\n' 'verify_competitor_benchmark_worksheet_for_evidence() {'
    printf '%s\n' '  local expected_commit_marker'
    printf '%s\n' '  local required_label'
    printf '%s\n' '  local required_value'
    printf '%s\n' '  expected_commit_marker="Source commit: \`$EXPECTED_SOURCE_COMMIT\`"'
    printf '%s\n' ''
    printf '%s\n' '  if [[ ! -f "$COMPETITOR_BENCHMARK_WORKSHEET_FILE" ]]; then'
    printf '%s\n' '    printf "BLOCKER: competitor benchmark worksheet is missing, stale, or incomplete: %s does not exist.\n" "$COMPETITOR_BENCHMARK_WORKSHEET_FILE" >&2'
    printf '%s\n' '    exit 2'
    printf '%s\n' '  fi'
    printf '%s\n' ''
    printf '%s\n' '  if ! grep -F -- "$expected_commit_marker" "$COMPETITOR_BENCHMARK_WORKSHEET_FILE" >/dev/null; then'
    printf '%s\n' '    printf "BLOCKER: competitor benchmark worksheet is missing, stale, or incomplete: expected release candidate source commit %s.\n" "$EXPECTED_SOURCE_COMMIT" >&2'
    printf '%s\n' '    exit 2'
    printf '%s\n' '  fi'
    printf '%s\n' ''
    printf '%s\n' '  if ! grep -Fx -- "Status: completed" "$COMPETITOR_BENCHMARK_WORKSHEET_FILE" >/dev/null; then'
    printf '%s\n' '    printf "BLOCKER: competitor benchmark worksheet is missing, stale, or incomplete: set Status: completed after filling benchmark findings.\n" >&2'
    printf '%s\n' '    exit 2'
    printf '%s\n' '  fi'
    printf '%s\n' ''
    printf '%s\n' '  if grep -Eq "(^Status:[[:space:]]*pending$|To Fill|This file is not release evidence|pending worksheet|not release evidence|Replace this pending worksheet|Do not copy this file)" "$COMPETITOR_BENCHMARK_WORKSHEET_FILE"; then'
    printf '%s\n' '    printf "BLOCKER: competitor benchmark worksheet is missing, stale, or incomplete: remove pending/template instructions before writing evidence.\n" >&2'
    printf '%s\n' '    exit 2'
    printf '%s\n' '  fi'
    printf '%s\n' ''
    printf '%s\n' '  if grep -F -- "- [ ]" "$COMPETITOR_BENCHMARK_WORKSHEET_FILE" >/dev/null; then'
    printf '%s\n' '    printf "BLOCKER: competitor benchmark worksheet is missing, stale, or incomplete: unchecked benchmark items remain.\n" >&2'
    printf '%s\n' '    exit 2'
    printf '%s\n' '  fi'
    printf '%s\n' ''
    printf '%s\n' '  for required_label in "Notion" "Todoist" "Linear" "Motion" "Ship" "Defer" "Reject"; do'
    printf '%s\n' '    required_value="$(awk -v label="$required_label" '\''index($0, "- " label ":") == 1 { value = $0; sub("^- " label ":[[:space:]]*", "", value); print value; exit }'\'' "$COMPETITOR_BENCHMARK_WORKSHEET_FILE")"'
    printf '%s\n' '    if [[ -z "${required_value//[[:space:]]/}" ]]; then'
    printf '%s\n' '      printf "BLOCKER: competitor benchmark worksheet is missing, stale, or incomplete: fill %s.\n" "$required_label" >&2'
    printf '%s\n' '      exit 2'
    printf '%s\n' '    fi'
    printf '%s\n' '    if competitor_worksheet_value_is_placeholder_or_boilerplate "$required_value"; then'
    printf '%s\n' '      printf "BLOCKER: competitor benchmark worksheet is missing, stale, or incomplete: fill %s with concrete competitor benchmark observation.\n" "$required_label" >&2'
    printf '%s\n' '      exit 2'
    printf '%s\n' '    fi'
    printf '%s\n' '  done'
    printf '%s\n' '}'
