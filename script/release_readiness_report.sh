...
AUTOMATED_PROOF_GATES="${SUISUI_AUTOMATED_PROOF_GATES:-0}"
...
RELEASE_CI_PREFLIGHT="${SUISUI_RELEASE_CI_PREFLIGHT:-$AUTOMATED_PROOF_GATES}"
RELEASE_CI_PREFLIGHT_RELATIVE="scripts/ci.sh"
LOCAL_CRUD_SMOKE="${SUISUI_LOCAL_CRUD_SMOKE:-$AUTOMATED_PROOF_GATES}"
LOCAL_CRUD_SMOKE_RELATIVE="script/check_local_crud_smoke.sh"
RUNTIME_ACCESSIBLE_CRUD_SMOKE="${SUISUI_RUNTIME_ACCESSIBLE_CRUD_SMOKE:-$AUTOMATED_PROOF_GATES}"
RUNTIME_ACCESSIBLE_CRUD_SMOKE_RELATIVE="script/check_runtime_accessible_crud_smoke.sh"
LAYOUT_STABILITY_SMOKE="${SUISUI_LAYOUT_STABILITY_SMOKE:-0}"
LAYOUT_STABILITY_SMOKE_RELATIVE="script/check_layout_stability_smoke.sh"
RELEASE_XCODE_PREFLIGHT="${SUISUI_RELEASE_XCODE_PREFLIGHT:-$AUTOMATED_PROOF_GATES}"
...
ACCESSIBILITY_RUNTIME_PREFLIGHT="${SUISUI_ACCESSIBILITY_RUNTIME_PREFLIGHT:-$AUTOMATED_PROOF_GATES}"
...
LOCAL_VOICE_EVIDENCE_RELATIVE="docs/release/evidence/local-voice-runtime.md"
LOCAL_VOICE_GENERATOR_MARKER="Generated by: script/check_local_voice_runtime_smoke.sh"
...
RUNTIME_SOURCE_DIRS=(
  "$ROOT_DIR/Sources/SuisuiCore"
  "$ROOT_DIR/Sources/SuisuiApp"
  "$ROOT_DIR/Sources/SuisuiCLI"
)
...
LOCAL_VOICE_REQUIRED_MARKERS=(
  "Status: passed"
  "STT transcript: passed"
  "TTS Japanese WAV: passed"
  "TTS English WAV: passed"
...
EXPECTED_AUTOMATED_PREFLIGHT_APP_NAME="${APP_NAME:-Suisui}"
EXPECTED_AUTOMATED_PREFLIGHT_XCODE_WORKSPACE="$XCODE_WORKSPACE_RELATIVE"
EXPECTED_AUTOMATED_PREFLIGHT_XCODE_SCHEME="Suisui"
EXPECTED_AUTOMATED_PREFLIGHT_XCODE_CONFIGURATION="Debug"
EXPECTED_AUTOMATED_PREFLIGHT_XCODE_DESTINATION="platform=macOS"
...
check_manual_unblocker_runbook_freshness() {
  ...
  local required_automated_evidence_bootstrap='SUISUI_AUTOMATED_PREFLIGHT_EVIDENCE_FILE="$automated_evidence" ./script/check_automated_release_preflight.sh'
...
ui_evidence_source_commit_full() {
  local commit
...
failure_reproduction_command() {
  ...
  case "$blocker_message" in
    *"release CI preflight"*)
      printf 'SUISUI_RELEASE_CI_PREFLIGHT=1 ./script/release_readiness_report.sh'
      ;;
    *"local CRUD smoke"*)
    ...
    *"runtime accessible CRUD smoke"*)
      ...
      ;;
    *"layout stability smoke"*)
      printf 'SUISUI_LAYOUT_STABILITY_SMOKE=1 ./script/release_readiness_report.sh'
      ;;
    *"release Xcode preflight"*)
      printf 'SUISUI_RELEASE_XCODE_PREFLIGHT=1 ./script/release_readiness_report.sh'
      ;;
    *"release launch preflight"*)
      printf 'SUISUI_RELEASE_LAUNCH_PREFLIGHT=1 ./script/release_readiness_report.sh'
      ;;
    *"accessibility runtime preflight"*)
      printf 'SUISUI_ACCESSIBILITY_RUNTIME_PREFLIGHT=1 ./script/release_readiness_report.sh'
      ;;
    *"MCP compliance"*)
      printf './script/verify_mcp_compliance.sh'
      ;;
    *"local voice runtime"*)
      printf 'SUISUI_LOCAL_VOICE_EVIDENCE_FILE=docs/release/evidence/local-voice-runtime.md SUISUI_STT_EXPECTED_TRANSCRIPT_CONTAINS="<expected words>" ./script/check_local_voice_runtime_smoke.sh'
      ;;
...
write_automated_proof_gate_actions() {
  ...
  if automated_preflight_evidence_covers "Release CI"; then
    ...
    printf -- "- Run: \`SUISUI_AUTOMATED_PROOF_GATES=1 ./script/release_readiness_report.sh\`\n"
    ...
    printf -- "- Optional override for non-standard evidence paths: \`SUISUI_AUTOMATED_PREFLIGHT_EVIDENCE_FILE=%s ./script/check_automated_release_preflight.sh\`\n" "$default_evidence_relative"
    printf -- "- Then rerun: \`./script/release_readiness_report.sh\` to auto-discover \`%s\` for the current source commit.\n" "$default_evidence_relative"
    printf -- "- Explicit reuse also works: \`SUISUI_AUTOMATED_PREFLIGHT_EVIDENCE_FILE=%s ./script/release_readiness_report.sh\`\n" "$default_evidence_relative"
  fi
  printf "\n"
}
...
write_competitor_hands_on_evidence_invocation() {
  ...
  printf '%s\n' '  --ship "<Suisui behavior to ship now based on the hands-on benchmark>" \'
...
write_release_evidence_login_item_command() {
  printf '%s\n' '```bash'
  printf '%s\n' 'export SUISUI_RELEASE_ARTIFACT_SHA256_FILE="<path to generated DMG .sha256>"'
...
voiceover_launch_env_contains_current_source_commit() {
  ...
  grep -Eq "^SUISUI_VOICEOVER_REVIEW_SOURCE_COMMIT=(\"$expected_commit\"|'$expected_commit'|$expected_commit)[[:space:]]*$" "$helper_path" &&
    grep -Eq "^SUISUI_VOICEOVER_REVIEW_PROJECT_ID=(\"[1-9][0-9]*\"|'[1-9][0-9]*'|[1-9][0-9]*)[[:space:]]*$" "$helper_path"
}
...
write_release_actions() {
  ...
  mkdir -p "$(dirname "$action_path")"
  {
    ...
    printf -- "- Release-machine failures should link to \`script/verify_release_environment.sh\` or \`Tests/SuisuiCoreTests/ReleasePipelineTests.swift\`.\n\n"
    ...
    printf -- "- The candidate writes \`.tmp/voiceover-review/launch.env\` with \`SUISUI_VOICEOVER_REVIEW_SOURCE_COMMIT\` and \`SUISUI_VOICEOVER_REVIEW_PROJECT_ID\`; verify those values before launching the manual VoiceOver pass.\n"
...
is_boilerplate_competitor_value() {
  ...
  case "$normalized" in
    ...
    "suisui behavior to ship now based on the hands on benchmark"|\
...
printf "Suisui release readiness report\n"

if [[ "$AUTOMATED_PROOF_GATES" != "0" && "$AUTOMATED_PROOF_GATES" != "1" ]]; then
  blocker "SUISUI_AUTOMATED_PROOF_GATES must be 0 or 1"
fi
...
if [[ "$RELEASE_CI_PREFLIGHT" != "0" && "$RELEASE_CI_PREFLIGHT" != "1" ]]; then
  blocker "SUISUI_RELEASE_CI_PREFLIGHT must be 0 or 1"
  ...
  printf "INFO: release CI preflight skipped; set SUISUI_RELEASE_CI_PREFLIGHT=1 to run %s inside this report.\n" "$RELEASE_CI_PREFLIGHT_RELATIVE"
...
if [[ "$LOCAL_CRUD_SMOKE" != "0" && "$LOCAL_CRUD_SMOKE" != "1" ]]; then
  blocker "SUISUI_LOCAL_CRUD_SMOKE must be 0 or 1"
  ...
  printf "INFO: local CRUD smoke skipped; set SUISUI_LOCAL_CRUD_SMOKE=1 to run %s inside this report.\n" "$LOCAL_CRUD_SMOKE_RELATIVE"
...
if [[ "$RUNTIME_ACCESSIBLE_CRUD_SMOKE" != "0" && "$RUNTIME_ACCESSIBLE_CRUD_SMOKE" != "1" ]]; then
  blocker "SUISUI_RUNTIME_ACCESSIBLE_CRUD_SMOKE must be 0 or 1"
  ...
  printf "INFO: runtime accessible CRUD smoke skipped; set SUISUI_RUNTIME_ACCESSIBLE_CRUD_SMOKE=1 to run %s against the visible app and isolated SQLite database.\n" "$RUNTIME_ACCESSIBLE_CRUD_SMOKE_RELATIVE"
...
if [[ "$LAYOUT_STABILITY_SMOKE" != "0" && "$LAYOUT_STABILITY_SMOKE" != "1" ]]; then
  blocker "SUISUI_LAYOUT_STABILITY_SMOKE must be 0 or 1"
  ...
  printf "INFO: layout stability smoke skipped; set SUISUI_LAYOUT_STABILITY_SMOKE=1 to run %s against the Project Board frame stability harness.\n" "$LAYOUT_STABILITY_SMOKE_RELATIVE"
...
if [[ "$RELEASE_XCODE_PREFLIGHT" != "0" && "$RELEASE_XCODE_PREFLIGHT" != "1" ]]; then
  blocker "SUISUI_RELEASE_XCODE_PREFLIGHT must be 0 or 1"
  ...
  printf "INFO: release Xcode preflight skipped; set SUISUI_RELEASE_XCODE_PREFLIGHT=1 to run xcodebuild against %s.\n" "$XCODE_WORKSPACE_RELATIVE"
...
if [[ "$RELEASE_LAUNCH_PREFLIGHT" != "0" && "$RELEASE_LAUNCH_PREFLIGHT" != "1" ]]; then
  blocker "SUISUI_RELEASE_LAUNCH_PREFLIGHT must be 0 or 1"
  ...
  printf "INFO: release launch preflight skipped; set SUISUI_RELEASE_LAUNCH_PREFLIGHT=1 to run %s --verify inside this report.\n" "$RELEASE_LAUNCH_PREFLIGHT_RELATIVE"
...
for visual_evidence_contract in "${UI_VISUAL_EVIDENCE_CONTRACTS[@]}"; do
  ...
  if [[ ! -f "$locale_evidence_file" || -L "$locale_evidence_file" ]]; then
    ...
    if ! grep -F 'Generated with `script/capture_ui_evidence.sh`.' "$locale_evidence_file" >/dev/null; then
      ui_blocker "$locale_name UI screenshot evidence file was not generated by script/capture_ui_evidence.sh"
    fi
    if ! grep -F -- "- Generated at:" "$locale_evidence_file" >/dev/null; then
      ui_blocker "$locale_name UI screenshot evidence is missing generated timestamp"
    fi
    ui_evidence_source_commit_value="$(
      normalize_markdown_context_value "$(markdown_context_value "$locale_evidence_file" "Source commit")"
    )"
    if [[ -z "$ui_evidence_source_commit_value" ]]; then
      ui_blocker "$locale_name UI screenshot evidence is missing source commit"
    elif [[ "$expected_ui_evidence_source_commit" != "unknown" && "$ui_evidence_source_commit_value" != "$expected_ui_evidence_source_commit" ]]; then
      ui_blocker "$locale_name UI screenshot evidence source commit does not match current UI source commit: expected $expected_ui_evidence_source_commit"
    fi
    runtime_context="$(
      normalize_markdown_context_value "$(markdown_context_value "$locale_evidence_file" "Runtime context")"
    )"
  ...
  if [[ ! -f "$capture_manifest_file" || -L "$capture_manifest_file" ]]; then
    ui_blocker "missing or invalid $locale_name visual capture manifest: $capture_manifest_relative"
    continue
  fi
  capture_manifest_source_commit="$(
    /usr/bin/plutil -extract sourceCommit raw -o - "$capture_manifest_file" 2>/dev/null || true
  )"
  ...
  if [[ -z "$capture_manifest_source_commit" ]]; then
    ...
    ui_blocker "$locale_name visual capture manifest source commit does not match current visual evidence source commit: expected $expected_ui_evidence_source_commit_full"
  fi
...
if [[ ! -x "$accessibility_preflight_script" ]]; then
  ...
  if [[ "$ACCESSIBILITY_RUNTIME_PREFLIGHT" != "0" && "$ACCESSIBILITY_RUNTIME_PREFLIGHT" != "1" ]]; then
    voiceover_blocker "SUISUI_ACCESSIBILITY_RUNTIME_PREFLIGHT must be 0 or 1"
  fi
  ...
  if [[ "$ACCESSIBILITY_RUNTIME_PREFLIGHT" == "1" ]]; then
    ...
    printf "INFO: accessibility runtime preflight skipped; set SUISUI_ACCESSIBILITY_RUNTIME_PREFLIGHT=1 to include the visible AX smoke check.\n"
...
local_voice_evidence_file="$ROOT_DIR/$LOCAL_VOICE_EVIDENCE_RELATIVE"
local_voice_evidence_blocker_count=0

local_voice_blocker() {
  local_voice_evidence_blocker_count=$((local_voice_evidence_blocker_count + 1))
  blocker "$1"
}

local_voice_context_value() {
  markdown_context_value "$local_voice_evidence_file" "$1"
}

normalize_local_voice_context_value() {
  normalize_markdown_context_value "$1"
}

if [[ ! -f "$local_voice_evidence_file" ]]; then
  local_voice_blocker "missing local voice runtime evidence file: $LOCAL_VOICE_EVIDENCE_RELATIVE"
else
  grep -Fx "Status: passed" "$local_voice_evidence_file" >/dev/null && local_voice_status_passed=1 || local_voice_status_passed=0
  for required_marker in "${LOCAL_VOICE_REQUIRED_MARKERS[@]}"; do
    if [[ "$required_marker" == "Status: passed" ]]; then
      if ! grep -Fx "Status: passed" "$local_voice_evidence_file" >/dev/null; then
        local_voice_blocker "local voice runtime evidence is not marked passed"
      fi
    elif ! grep -F "$required_marker" "$local_voice_evidence_file" >/dev/null; then
      local_voice_blocker "local voice runtime evidence is missing marker: $required_marker"
    fi
  done

  if grep -Eiq '(pending|todo|tbd|placeholder|sample|example|replace me)' "$local_voice_evidence_file"; then
    local_voice_blocker "local voice runtime evidence still contains pending/template/placeholder text"
  fi
  if grep -F -- '- [ ]' "$local_voice_evidence_file" >/dev/null; then
    local_voice_blocker "local voice runtime evidence still contains unchecked checklist markers"
  fi
  if grep -Eiq '(/Users/|/Volumes/|file://)' "$local_voice_evidence_file"; then
    local_voice_blocker "local voice runtime evidence contains local path text"
  fi

  for context_label in "${LOCAL_VOICE_REQUIRED_CONTEXT_LABELS[@]}"; do
    context_value="$(local_voice_context_value "$context_label")"
    compact_context_value="$(tr -d '[:space:]' <<<"$context_value")"
    if [[ -z "$compact_context_value" ]]; then
      local_voice_blocker "local voice runtime evidence missing runtime context: $context_label"
      continue
    fi
    if grep -Eiq '(pending|todo|tbd|placeholder|sample|example|replace me)' <<<"$context_value"; then
      local_voice_blocker "local voice runtime evidence has template runtime context: $context_label"
    fi
    if [[ "$context_label" == "Generated at" && "$context_value" != *T*Z ]]; then
      local_voice_blocker "local voice runtime evidence has invalid generated timestamp"
    fi
    if [[ "$context_label" == "STT expected transcript marker" ]] && is_boilerplate_voiceover_note "$context_value"; then
      local_voice_blocker "local voice runtime evidence has boilerplate STT transcript marker"
    fi
  done

  if [[ "$local_voice_status_passed" -eq 1 ]]; then
    if ! grep -Fx "$LOCAL_VOICE_GENERATOR_MARKER" "$local_voice_evidence_file" >/dev/null; then
      local_voice_blocker "local voice runtime evidence was not generated by script/check_local_voice_runtime_smoke.sh"
    fi

    expected_source_commit="$(local_voice_evidence_source_commit)"
    local_voice_source_commit="$(normalize_local_voice_context_value "$(local_voice_context_value "Source commit")")"
    if [[ -n "$local_voice_source_commit" && "$local_voice_source_commit" != "$expected_source_commit" ]]; then
      local_voice_blocker "local voice runtime evidence source commit does not match current local voice source commit: expected $expected_source_commit"
    fi
  fi

  tts_languages_context="$(normalize_local_voice_context_value "$(local_voice_context_value "TTS languages")")"
  if [[ "$tts_languages_context" != *ja* || "$tts_languages_context" != *en* ]]; then
    local_voice_blocker "local voice runtime evidence must cover both Japanese and English TTS"
  fi
fi
if [[ "$local_voice_evidence_blocker_count" -eq 0 ]]; then
  printf "OK: local voice runtime evidence covers checksum-verified STT and Japanese/English TTS runtime proof\n"
else
  printf "NEXT: run \`SUISUI_LOCAL_VOICE_EVIDENCE_FILE=%s SUISUI_STT_EXPECTED_TRANSCRIPT_CONTAINS=\"<expected words>\" ./script/check_local_voice_runtime_smoke.sh\` with checksum-verified ggml-tiny.bin, whisper-cli, kokoro-v1_0.pth, Kokoro runtime, and a real STT sample WAV; commit the generated evidence only after it is path-redacted and passed.\n" "$LOCAL_VOICE_EVIDENCE_RELATIVE"
fi
