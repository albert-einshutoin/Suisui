...
guard AXIsProcessTrusted() else {
    fputs("Accessibility permission is required to count SoloPM UI evidence markers.\n", stderr)
    exit(2)
}
...
if visitedCount >= maxNodes {
    fputs("AX identifier count reached SOLOPM_UI_EVIDENCE_AX_MAX_NODES=\(maxNodes).\n", stderr)
    exit(2)
}
