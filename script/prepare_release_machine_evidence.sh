...
write_command() {
...

  {
    ...
    printf '%s\n' '  fi'
    printf '%s\n' ''
    printf '%s\n' '  if grep -Eq "(Status:[[:space:]]*pending|To Fill|^## Closeout$|This worksheet is not release evidence|Replace every placeholder|not release evidence)" "$RELEASE_MACHINE_WORKSHEET_FILE"; then'
    printf '%s\n' '    printf "BLOCKER: release-machine worksheet is missing, stale, or incomplete: remove pending/template instructions before writing evidence.\n" >&2'
    printf '%s\n' '    exit 2'
    printf '%s\n' '  fi'
    printf '%s\n' ''
    ...
    printf '%s\n' '    if ! grep -Eq "^- ${required_label}:[[:space:]]*[^[:space:]]" "$RELEASE_MACHINE_WORKSHEET_FILE"; then'
