...
enum LaunchPerformanceSignposts {
    static let signposter = OSSignposter(subsystem: "dev.suisui.performance", category: "launch")
...
enum LaunchPerformanceMilestones {
    ...
    static func record(_ label: String) {
        guard let path = ProcessInfo.processInfo.environment["SUISUI_LAUNCH_TIMELINE_PATH"],
              !path.isEmpty else {
            return
        }
