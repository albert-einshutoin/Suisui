...
extension Notification.Name {
    ...
    static let suisuiDigestNotificationOpened = Notification.Name("dev.suisui.digestNotificationOpened")
}
...
final class SuisuiNotificationResponder: NSObject, UNUserNotificationCenterDelegate, @unchecked Sendable {
    static let shared = SuisuiNotificationResponder()
    ...
    // Deadline notifications stay useful while Suisui is frontmost; a menu
    ...
    func userNotificationCenter(
        ...
        if actionIdentifier == UNNotificationDefaultActionIdentifier,
           requestIdentifier.hasPrefix("suisui-daily-digest-")
               || requestIdentifier.hasPrefix("suisui-weekly-review-") {
#if canImport(AppKit)
            DispatchQueue.main.async {
                NSApp.activate(ignoringOtherApps: true)
                NotificationCenter.default.post(name: .suisuiDigestNotificationOpened, object: nil)
            }
#endif
        }
...
final class DockTileBadgeController {
    ...
    func start() {
        ...
        observer = NotificationCenter.default.addObserver(
            forName: .suisuiProjectBoardDidChange,
            object: nil,
            queue: .main
        ) { _ in
            Task { @MainActor in
