...
public struct ProcessDevelopmentCommandRunner: DevelopmentCommandRunner {
    ...
    public func run(executable: String, arguments: [String], workingDirectory: URL) throws -> DevelopmentCommandOutput {
        ...
        let tmpRoot = workingDirectory
            ...
            .appendingPathComponent("solopm-verification", isDirectory: true)
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let homeRoot = workingDirectory
            .appendingPathComponent(".tmp", isDirectory: true)
            .appendingPathComponent("solopm-home", isDirectory: true)
        ...
        // environment narrow so API/provider tokens from SoloPM are not inherited.
        process.environment = [
            "HOME": homeRoot.path,
            "LANG": "C",
            "LC_ALL": "C",
...
public enum DevelopmentVerificationCommandPolicy {
    ...
    private static let commands: [String: DevelopmentVerificationCommand] = [
        ...
        "swift.build.solopm_cli": DevelopmentVerificationCommand(
            id: "swift.build.solopm_cli",
            executable: "swift",
            arguments: ["build", "--product", "solopm-cli"]
        ),
