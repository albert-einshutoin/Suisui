...
public enum CoreMigrations {
    ...
    public static var current: [DatabaseMigration] {
        phase9 + [
            ...
            DatabaseMigration(id: "0027_add_task_context_fact_policy") { connection in
                try connection.execute(
                    """
                    PRAGMA defer_foreign_keys = ON;

                    CREATE TABLE task_context_facts_policy (
                        id TEXT PRIMARY KEY NOT NULL,
                        session_id TEXT NOT NULL,
                        kind TEXT NOT NULL CHECK(kind IN (
                            'goal',
                        ...
                        supersedes_fact_id TEXT
                            REFERENCES task_context_facts_policy(id)
                            ON DELETE SET NULL,
                        expires_at REAL,
                        created_at REAL NOT NULL,
                    ...
                    INSERT INTO task_context_facts_policy (
                        id,
                        session_id,
                        kind,
                        scope_kind,
                    ...
                    SELECT
                        ...
                        confidence,
                        CASE author
                            WHEN 'system_derived' THEN 'deterministic'
                            ELSE author
                        END,
                        supersedes_fact_id,
                        NULL,
