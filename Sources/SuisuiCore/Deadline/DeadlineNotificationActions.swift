...
public enum DeadlineNotificationInteraction {
    ...
    public static let taskCategoryIdentifier = "suisui.deadline.task"
    /// userInfo key holding the local task id the notification refers to.
    public static let taskIDUserInfoKey = "suisui.task.id"
    /// Action: mark the referenced task as completed.
    public static let completeTaskActionIdentifier = "suisui.deadline.action.complete-task"
    /// Action: reschedule the same notification one hour from now.
    public static let snoozeOneHourActionIdentifier = "suisui.deadline.action.snooze-1h"
        ...
        do {
            ...
            let record = try notificationClient.schedule(
                NotificationDraft(
                    ...
                    identifierHint: "suisui-snooze-\(taskID)-\(scheduledAt)",
