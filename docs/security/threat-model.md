...
| Codex executable substitution | Approval and both process launch paths compare streaming SHA-256 plus macOS signing identity. Stable compiles and evaluates an Apple-anchored Developer ID requirement that pins OpenAI Team ID `2DC432GLL2` and identifier `codex`; matching metadata strings without that certificate chain are rejected. Same-size edits, restored mtimes, symlink retargets, signature changes, and package-manager replacements fail closed until explicit reapproval. |
