...
| App Shell and Runtime Composition | Application launch, scene wiring, migrations, store factories, provider factories, settings runtime factories, and platform permission snapshots. | `Sources/SoloPMApp/SoloPMApp.swift`, `Sources/SoloPMApp/Composition/*`, `Sources/SoloPMApp/Adapters/*`, `Sources/SoloPMCore/Database/*` | Owns composition roots and platform adapter selection. It may instantiate SQLite, Keychain, EventKit, UserDefaults, and OAuth/network runtime services when kept out of feature view files. |
...
- `Sources/SoloPMApp/SoloPMApp.swift` remains the app scene composition root.
  Runtime construction lives in `Sources/SoloPMApp/Composition/*`, where SQLite
