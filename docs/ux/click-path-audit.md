...
| Schedule Calendar apply | app launch -> sidebar `Schedule` -> `Generate Draft` -> `Queue Calendar Apply` -> Pending Actions approval | Reachable. Queueing immediately hands the user to the canonical Pending Actions review surface; Calendar remains unchanged before approval. | #615 / PR #622 |
...
| Voice Quick Capture planning or capture | app launch -> Voice Quick Capture -> record or type -> `Save to Inbox` / `Generate Plan` -> Inbox or Pending Actions review | Reachable. The single surface explains record/type -> Inbox or reviewable proposal; Conversation is not a second product tab. | #614 / PR #621; Core Value Loop runtime proof in #617 / PR #624 |
...
| Core Value Loop proof | clean source checkout -> Inbox capture/triage -> Voice Quick Capture interpretation/review -> Schedule proposal/Pending Actions -> receipt and recovery unit checks | Reachable and fail-closed. Runtime evidence uses only the normal product routes; Conversation visual evidence is excluded, and the manifest stores source commit plus redacted stage status only. | #617 / PR #624 |
...
| Schedule apply after draft generation | The draft must remain local until the user reviews the proposed Calendar writes. | source + runtime schedule smoke + visual | Fixed in #615 / PR #622; queueing opens Pending Actions and still routes Calendar writes through the execution gate. |
...
| Voice Quick Capture first-run path | The surface must teach that users can record or type, then either save to Inbox or generate an approval-reviewed proposal. | source + localization + runtime AX + visual | Fixed in #614 / PR #621; #617 / PR #624 proves the normal route with AX-stable identifiers and no pre-approval writes. |
