...
対象: 現在の SwiftUI 実装である `SoloPMApp.swift`, `ProjectBoardView.swift`, `ProjectWorkflow*View.swift`, `SettingsView.swift`, `MenuBarPanel.swift`, `VoiceCaptureView.swift` の主要操作。
...
| Task card metadata stripはhosted runtimeで改善済み | Task card metadata strip はstatus / priority / due / recurrenceを単一の意味的なTextへ統合する。drag affordanceは右上の固定サイズiconとして残り、Open task領域とstatus move controlsから分離済み。 | Done | Hosted Light/Dark/Systemスクリーンショットで、選択カードを含めtitle、状態、優先度、期限、drag affordanceが欠落・重複しないことを確認する。 |
...
| Task card metadata strip | titleとdrag affordanceを分離し、status / priority / due / recurrenceを単一の意味的なTextとして狭いcolumnでも読み切れるようにする。 | `Sources/SoloPMApp/Views/ProjectBoardDetailViews.swift` | `AppExperienceSourceTests.testTaskCardsUseSampleInspiredNonOverlappingMetadataStrip`, `ProjectBoardMetadataLayoutSourceTests` |
