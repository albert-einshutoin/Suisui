...
| Task card metadata strip | titleとdrag affordanceを分離し、status / priorityとdue / recurrenceを最大2行の意味的なTextとして狭いcolumnでも読み切れるようにする。 | `Sources/SoloPMApp/Views/ProjectBoardDetailViews.swift` | `AppExperienceSourceTests.testTaskCardsUseSampleInspiredNonOverlappingMetadataStrip`, `ProjectBoardMetadataLayoutSourceTests` |
