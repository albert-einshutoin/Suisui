...
Local TTS now exposes a ready-gated Kokoro surface in Settings. The provider is visible as `Local Kokoro`, but speech synthesis remains unavailable until both local runtime requirements are ready:

- The `kokoro-v1_0.pth` model is installed and checksum-verified.
- Settings has an absolute, executable Kokoro runtime path.

The first Kokoro provider slice is intentionally short-prompt only. It re-verifies the cached model before local synthesis and passes prompt text through a short-lived UTF-8 file instead of command-line arguments so task details are not exposed through process inspection. Full playback UX, bundled runtime packaging, and signed/notarized smoke evidence remain follow-up work.
