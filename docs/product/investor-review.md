...
| Paid value | Passed source/product evidence | Settings/Sync/MCP surfaces explain Pro value without blocking free CRUD. Sync shows Pro value and local-only safety before the toggle; MCP shows Pro execution value while preserving Free registration/check and approval safety. |
