# Suisui Pricing and Packaging

Verified: 2026-06-21

## Positioning

Suisui should keep the local Mac app useful without a subscription, then charge for cloud-backed convenience and safety controls.

The strongest paid message is:

> Create and capture tasks even when your Mac is offline or sleeping.

This is not just "remote MCP." The user-facing value is that Suisui can receive a voice command, mobile shortcut, webhook, or LLM tool request while the primary Mac is unavailable, then sync the resulting task or pending action back to every device.
...
- Suisui should not pay for general LLM inference. Users bring provider keys unless a later hosted AI plan is explicitly introduced. The managed AI add-on design (credits, rate cards, auto routing, relay metering) lives in `docs/product/ai-billing-model.md`.
...
| --- | ---: | --- | --- |
| Free | $0 | Local Mac app, local Project/Task/Knowledge storage, BYOK LLM providers, local Suisui-mediated tool execution, local MCP registration and diagnostics, manual export/import | No device sync, no cloud relay, no hosted MCP endpoint |
...
| Pro | $10/month or $96/year | Sync plus Cloud Relay, Hosted MCP endpoint, PC-off task capture, document-scoped automation, remote pending-action inbox, execution audit log, Suisui Harness, higher history/retention limits | Remote write actions still require policy and approval rules |
...
| `advancedMCPExecution` | Pro | Suisui calling registered external MCP tools after approval. |
...
Obsidian's current public model is a useful reference point: the core app is free, and Sync is a paid add-on starting at $4/month billed annually or $5/month billed monthly, with end-to-end encryption and version history. Suisui should use the same mental model but charge Pro for remote relay and harness features because those create a larger security and infrastructure burden.
...
- Use Suisui's internal tool registry and approval flow.
...
> I use Suisui on iOS, Web, and macOS and want the same tasks, projects, conversations, and settings everywhere.
...
> Suisui can accept task capture and automation requests even when your Mac is not running.
...
- A repeatable workflow test and automation harness for Suisui actions.
...
Suisui should expose three clearly different modes:

| Mode | Plan | Description |
| --- | --- | --- |
| Suisui-mediated local tools | Free | LLM output is converted into an Action Plan, reviewed locally, then executed by Suisui. |
| External MCP client execution | Pro execution gate | Suisui calls user-registered MCP servers with approval and audit controls. Free can register and diagnose. |
| Hosted MCP / Cloud Relay | Pro | External LLMs or clients call a Suisui-hosted HTTPS endpoint so requests can be captured while devices are unavailable. |
...
↓
Suisui on Mac/iPhone/iPad syncs later
↓
Task appears in Inbox, or pending action waits for approval
```

This is the core paid wedge. It keeps the local-first promise for normal use while giving paying users a reason to trust Suisui as an always-available personal PM layer.

## Multiplatform Direction

Suisui's long-term packaging assumes three product surfaces:
...
- "Bring your own AI key; pay Suisui only for sync, relay, and safety infrastructure."
