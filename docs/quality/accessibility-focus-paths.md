...
| Move to In Progress | `task-status-move-in_progress` | button | Proves at least one concrete status move button is present, labeled with its target status, and reachable after task edits. |
