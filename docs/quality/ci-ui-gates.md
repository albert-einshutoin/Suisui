...
SwiftPM jobは成功・失敗に関係なく、discovered/executed/skipped件数、sanitized test log、test name inventory、xUnit XMLを`.tmp/ci-artifacts/swiftpm`へ7日間保存する。0件、committed baseline未満、件数を抽出できない結果はfail closedとし、retryでgreenへ変えない。
