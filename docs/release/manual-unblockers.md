# Manual Release Unblockers

Date: 2026-06-21

Current automated evidence: `.tmp/automated-release-preflight-5875b4e.md`

Current release-candidate source commit: `17880b5`

Use this runbook after `release_readiness_report.sh` is green for automated gates and still reports manual/release-machine blockers. Do not hand-edit passed evidence. The release report rejects hand-written pass files and stale source commits. Run `--validate-only` before writing tracked evidence, then rerun readiness with the current automated preflight evidence.

```bash
SOLOPM_AUTOMATED_PREFLIGHT_EVIDENCE_FILE=.tmp/automated-release-preflight-5875b4e.md ./script/release_readiness_report.sh
```

## VoiceOver

Goal: replace stale `docs/release/evidence/accessibility-voiceover.md` with a real manual VoiceOver pass for release-candidate source commit `17880b5`.

Generated helpers:

- `.tmp/voiceover-review/accessibility-voiceover-pending-17880b5.md`
- `.tmp/voiceover-review/voiceover-worksheet.md`
- `.tmp/voiceover-review/create-evidence-command.sh`

Required manual work:

1. Open the candidate from `.tmp/voiceover-review/launch.env` or rerun the launch command printed by `./script/prepare_release_manual_helpers.sh`.
2. With actual VoiceOver enabled, verify Project board -> card -> Inline Task Composer -> inspector focus order.
3. Fill `.tmp/voiceover-review/voiceover-worksheet.md` with concrete observations. Do not use placeholders such as `TBD`, `Verified`, `OK`, or `No issues`.
4. Run the generated command in validate-only mode first.
5. Run the generated command in write mode only after validation succeeds.

Release-ready evidence requirements:

- `Generated by: script/create_voiceover_evidence.sh`
- `Status: passed`
- release-candidate `Source commit: 17880b5`
- real macOS version, app build, reviewer, check date, evidence source, and accessibility environment
- runtime AX smoke line with `unlabeledButtons=0`, `genericButtons=0`, `crudSignals=8/8`, and `focusPathSignals=6/6`
- concrete notes for project navigation, project board detail, open task, Inline Task Composer, status controls, task inspector, save changes, delete confirmation, no keyboard trap, and no unlabeled CRUD controls

Verification:

```bash
./.tmp/voiceover-review/create-evidence-command.sh --validate-only
SOLOPM_AUTOMATED_PREFLIGHT_EVIDENCE_FILE=.tmp/automated-release-preflight-5875b4e.md ./script/release_readiness_report.sh
```

Commit only after the VoiceOver section is green:

```bash
git add docs/release/evidence/accessibility-voiceover.md
git commit -m "docs: record current voiceover release evidence"
```

## Competitor hands-on

Goal: replace pending competitor evidence and convert `docs/product/competitor-benchmark.md` from worksheet/desk research to real hands-on findings.

Generated helpers:

- `.tmp/competitor-hands-on/competitor-hands-on-pending-17880b5.md`
- `.tmp/competitor-hands-on/hands-on-worksheet.md`
- `.tmp/competitor-hands-on/competitor-benchmark-pending-17880b5.md`
- `.tmp/competitor-hands-on/create-evidence-command.sh`

Required manual work:

1. Spend 2-4 hours total across Notion, Todoist, Linear, and Motion.
2. Fill `.tmp/competitor-hands-on/hands-on-worksheet.md` with concrete setup, capture, status movement, recommendation, and repeated-operation observations.
3. Fill `.tmp/competitor-hands-on/competitor-benchmark-pending-17880b5.md` with final hands-on findings and Ship / Defer / Reject deltas.
4. Run the generated command in validate-only mode first.
5. Run the generated command in write mode only after validation succeeds.

Release-ready evidence requirements:

- `Generated by: script/create_competitor_hands_on_evidence.sh`
- `Status: passed`
- release-candidate `Source commit: 17880b5`
- reviewer, check date, environment, and elapsed hands-on time
- concrete Notion, Todoist, Linear, and Motion notes
- explicit Ship, Defer, and Reject deltas
- `docs/product/competitor-benchmark.md` includes `## Hands-On Findings` and `Ship / Defer / Reject`

Verification:

```bash
./.tmp/competitor-hands-on/create-evidence-command.sh --validate-only
SOLOPM_AUTOMATED_PREFLIGHT_EVIDENCE_FILE=.tmp/automated-release-preflight-5875b4e.md ./script/release_readiness_report.sh
```

Commit only after the competitor section is green:

```bash
git add docs/release/evidence/competitor-hands-on.md docs/product/competitor-benchmark.md
git commit -m "docs: record competitor hands-on release evidence"
```

## Release machine

Goal: produce a signed, notarized, stapled release artifact with production Sparkle metadata and release evidence.

Generated helpers:

- `.tmp/release-machine/release-machine-worksheet.md`
- `.tmp/release-machine/create-release-evidence-command.sh`

Required local-only files on the release machine:

- `packaging/signing.env`
- `packaging/notarization.env`
- `packaging/sparkle.env`

Required environment/config values:

- `SOLOPM_SIGNING_IDENTITY`
- `SOLOPM_NOTARY_PROFILE`
- `SOLOPM_SPARKLE_FEED_URL`
- `SOLOPM_SPARKLE_PUBLIC_ED_KEY`
- `SOLOPM_SPARKLE_DOWNLOAD_URL_PREFIX`

Required release-machine flow:

```bash
./script/verify_signing_setup.sh
./script/sign_app.sh
SOLOPM_RELEASE_PREFLIGHT_ONLINE=1 ./script/verify_notarization_setup.sh
./script/notarize_app.sh
SOLOPM_PACKAGE_FORMAT=all ./script/package_release.sh
SOLOPM_REQUIRE_RELEASE_APPCAST=1 ./script/generate_appcast.sh
SOLOPM_REQUIRE_RELEASE_APPCAST=1 ./script/verify_appcast.sh dist/releases/appcast.xml
```

Then fill `.tmp/release-machine/release-machine-worksheet.md` while checking the signed/notarized app. The note must name the observed proof for each checked flag:

- release-machine launch
- checksum verification
- clean DMG install
- Applications folder install
- Gatekeeper acceptance
- clean environment launch
- login item toggle
- Sparkle appcast metadata

Release-ready evidence requirements:

- `generator.name: script/create_release_evidence.sh`
- signed/notarized/stapled `dist/SoloPM.app`
- production Sparkle `SUFeedURL` and `SUPublicEDKey`
- appcast with Sparkle `edSignature`
- matching `*.sha256` and `*.package-evidence.json`
- `packaging/release-evidence.json` generated by `script/create_release_evidence.sh`

Verification:

```bash
./.tmp/release-machine/create-release-evidence-command.sh --validate-only
./.tmp/release-machine/create-release-evidence-command.sh --force
SOLOPM_RELEASE_PREFLIGHT_ONLINE=1 ./script/verify_release_environment.sh
SOLOPM_AUTOMATED_PREFLIGHT_EVIDENCE_FILE=.tmp/automated-release-preflight-5875b4e.md ./script/release_readiness_report.sh
```

Commit only after release environment and readiness are green:

```bash
git add packaging/release-evidence.json
git commit -m "release: record signed release machine evidence"
```

## Final release check

After all three lanes are green:

```bash
swift test
SOLOPM_AUTOMATED_PREFLIGHT_EVIDENCE_FILE=.tmp/automated-release-preflight-5875b4e.md ./script/release_readiness_report.sh
git status --short --branch
```

Expected final state:

- `swift test` passes.
- `release_readiness_report.sh` reports ready.
- no pending/template/manual placeholder text remains in tracked evidence.
- release commits are pushed to `develop`.
