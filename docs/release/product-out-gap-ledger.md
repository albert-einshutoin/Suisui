...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Google Calendar live sync | [#3](https://github.com/albert-einshutoin/soloPM/issues/3), [#11](https://github.com/albert-einshutoin/soloPM/issues/11) | Blocker | Release owner | `SOLOPM_RUNTIME_SETTINGS_SAVE_TIMEOUT_SECONDS=90 ./script/check_runtime_settings_save_smoke.sh`, then real Google OAuth connect/list/select/write smoke | Capture credential-backed OAuth consent, calendar picker, saved calendar ID, and approved event write evidence with a test calendar. | `docs/release/public-alpha.md` if live sync is kept out of alpha | Phase16 connector reliability and sync onboarding if full Google production sync is deferred |
