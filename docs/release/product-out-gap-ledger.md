...
This ledger tracks current product-out work lanes that must be closed before SoloPM can be treated as a public release candidate. It is not release evidence; use it to keep blockers, accepted risks, and deferred Phase16/17 work explicit while `script/release_readiness_report.sh` and the evidence files remain the source of truth.
