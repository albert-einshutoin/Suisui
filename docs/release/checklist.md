...
The runtime preflight is intentionally stricter than a process/window check: it scans visible windows by AX role and fails if the best release-candidate window has fewer than the minimum buttons, text fields, or static texts, if it exposes unlabeled AX buttons, if a button only reports the generic `button` label without help or child text, if the visible Project Board does not expose the expected primary CRUD AX identifier/help signals as `crudSignals=8/8`, or if it does not expose the VoiceOver focus path AX identifier/help signals as `focusPathSignals=6/6`. The focus path signals cover Project navigation, Project board detail, Open task, Inline Task Composer, Status controls, and Task inspector so a low-information AX tree is not enough evidence for the manual VoiceOver pass.
