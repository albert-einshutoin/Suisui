...
When automated gates are green but readiness still reports manual/release-machine blockers or product-research advisories, use `docs/release/manual-unblockers.md` as the focused lane checklist for VoiceOver, competitor hands-on, and release-machine evidence. VoiceOver and release-machine failures remain blocking. Competitor hands-on is an advisory Product Research lane for Public Alpha and does not weaken blocking distribution, security, runtime, data-integrity, or accessibility gates.
...
The generated release evidence command requires a clean tracked source tree, pins the release evidence source commit it was created for, and exits before writing evidence if the worktree is dirty or has moved to another commit. Rerun `./script/prepare_release_manual_helpers.sh` after any source commit changes so release evidence, package evidence, and manual worksheet observations stay bound to the same release candidate.
The generated release-machine command also verifies `.tmp/release-machine/release-machine-worksheet.md` is marked completed, pinned to the same source commit, free of unchecked/pending/template markers, and filled before release evidence validation or writing can run.
...
./script/create_voiceover_evidence.sh --validate-only \
  ...
  --inbox-voice-triage-note "Concrete VoiceOver observation for Inbox transcript, interpretation, metadata, memo, and triage actions." \
  --today-rail-actions-note "Concrete VoiceOver observation for Today rail next action, focus, schedule, edit, subtask, and reminder controls." \
...
12. Competitor hands-on evidence (advisory Product Research)

Before checking the Phase 11 competitor benchmark item, replace `docs/release/evidence/competitor-hands-on.md` with the real 2-4 hour hands-on record. Update `docs/product/competitor-benchmark.md` from worksheet/desk research to hands-on findings before claiming Product Research complete. Missing, malformed, or stale competitor evidence remains visible as an advisory and produces `ready_with_advisories` when every blocking gate passes. The final evidence file must use `Status: passed`, complete reviewer/date/source/environment context, include the Notion -> Todoist -> Linear -> Motion path, document the Ship / Defer / Reject Delta, and explicitly confirm that no external SaaS sync or team workflow was added to public alpha scope.
...
- `blocked`: at least one distribution, security, accessibility, product-runtime, data-integrity, release-machine, or required automated gate failed; exit is non-zero.
- `ready_with_advisories`: all blocking gates passed, but competitor Product Research remains incomplete; Public Alpha exit is zero.
- `ready`: blocking gates and advisories are complete.

Use `--strict-product-research` for a dedicated product review that promotes incomplete competitor research to a non-zero result. This mode does not change the default Public Alpha policy. Set `SUISUI_RELEASE_STATUS_JSON_FILE=.tmp/release-status.json` when automation needs the schema-versioned status, blocking failures, advisories, source commits, and artifact identity.
...
A standard run writes `.tmp/automated-release-preflight-$(git rev-parse --short=8 HEAD).md` and requires a clean tracked source tree, so commit or discard tracked changes before producing release proof. Override `SUISUI_AUTOMATED_PREFLIGHT_EVIDENCE_FILE` only for a non-standard regular-file path below the repository `.tmp` directory; release proof never follows external or symlink destinations. The release readiness report auto-discovers `.tmp/automated-release-preflight-<commit>.md` for the current source commit when the environment variable is omitted.
The Xcode build gate has a fail-closed watchdog so a local SwiftBuild hang cannot leave release proof generation running forever. Use `SUISUI_XCODE_PREFLIGHT_TIMEOUT_SECONDS=<seconds>` only when a release machine is known to need a longer Xcode build window. If the Xcode watchdog times out, reproduce the exact package-root `xcodebuild -scheme ... -destination ... build` command from the blocker output and keep automated preflight evidence rejected until that Xcode build gate passes.
...
SUISUI_RELEASE_STATUS_JSON_FILE=.tmp/release-status.json ./script/release_readiness_report.sh
./script/release_readiness_report.sh --strict-product-research
