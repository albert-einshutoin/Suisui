...
The runtime preflight is intentionally stricter than a process/window check: it scans visible windows by AX role and fails if the best release-candidate window has fewer than the minimum buttons, text fields, or static texts, if it exposes unlabeled AX buttons, if a button only reports the generic `button` label without help or child text, or if the visible Project Board does not expose the expected primary CRUD button help signals as `crudSignals=8/8`, because a low-information AX tree is not enough evidence for the manual VoiceOver pass.
