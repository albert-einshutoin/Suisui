...
The adapter uses a configurable timeout with a 60-second default and does not retry automatically in Phase 1. The default model id is `gpt-5.5`, sourced through `LLMProviderCatalog` / `OpenAIResponsesConfiguration`; missing model settings must not fall through to an implicit provider-side or unintended model.
