...
class ExecutionContractTests(unittest.TestCase):
    ...
    def test_all_runner_adds_every_ui_lane_to_full_validation_with_pinned_helpers(self) -> None:
        ...
        self.assertEqual(workflow.count(performance_artifact), 1)
        self.assertIn("artifact-ids: ${{ needs.ui-performance-build.outputs.artifact-id }}", workflow)
    ...
    def test_mcp_verifier_resolves_npx_without_caller_path(self) -> None:
        ...
        self.assertNotIn("-u NODE_OPTIONS", contents)
        ...
        self.assertIn("NPM_CONFIG_IGNORE_SCRIPTS=true", contents)
        self.assertIn('NPM_CONFIG_CACHE="$WORK_DIR/npm-cache"', contents)
        self.assertIn('NPM_CONFIG_GLOBALCONFIG="$WORK_DIR/npm-globalconfig"', contents)
        self.assertIn("NPM_CONFIG_OFFLINE=false", contents)
        self.assertIn("NPM_CONFIG_PREFER_OFFLINE=false", contents)
        self.assertIn("NPM_CONFIG_UPDATE_NOTIFIER=false", contents)
        self.assertIn("TRUSTED_NODE_COMMAND=(", contents)
        self.assertRegex(contents, r"/usr/bin/env\s+-i")
        self.assertIn('HOME="$WORK_DIR/home"', contents)
        self.assertIn('TMPDIR="$WORK_DIR/tmp"', contents)
        ...
        self.assertIn('MCP_TEMP_ROOT="$ROOT_CANONICAL/.tmp"', contents)
        self.assertIn('[[ -L "$MCP_TEMP_ROOT"', contents)
        self.assertIn('pwd -P)" != "$MCP_TEMP_ROOT"', contents)
