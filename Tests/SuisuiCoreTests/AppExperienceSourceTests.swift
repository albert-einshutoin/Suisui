...
final class AppExperienceSourceTests: XCTestCase {
    ...
    func testProjectBoardHeaderLayoutRuntimeSmokeCoversNativeChromeMinimumWidthAndUtilities() throws {
        ...
        let toolbarStart = try XCTUnwrap(script.range(of: "assert_single_native_toolbar() {"))
        let toolbarEnd = try XCTUnwrap(script.range(of: "\n}\n", range: toolbarStart.upperBound..<script.endIndex))
        let toolbarSource = String(script[toolbarStart.lowerBound..<toolbarEnd.lowerBound])
        XCTAssertTrue(toolbarSource.contains("project-board-sidebar-toggle"))
        XCTAssertTrue(toolbarSource.contains("project-board-integrations-menu"))
        XCTAssertFalse(toolbarSource.contains("project-board-detail"))
        ...
        let sidebarStart = try XCTUnwrap(script.range(of: "exercise_sidebar_entrypoints() {"))
        let sidebarEnd = try XCTUnwrap(script.range(of: "\n}\n", range: sidebarStart.upperBound..<script.endIndex))
        let sidebarSource = String(script[sidebarStart.lowerBound..<sidebarEnd.lowerBound])
        XCTAssertFalse(sidebarSource.contains("launch_header_layout_candidate"))
        XCTAssertFalse(sidebarSource.contains("command-palette-row-project"))
        XCTAssertEqual(script.components(separatedBy: "exercise_toolbar_utilities\nexercise_sidebar_entrypoints\nexercise_settings_utility").count - 1, 2)
        XCTAssertTrue(sidebarSource.contains("key code 53"))
        XCTAssertTrue(sidebarSource.contains("wait_for_process_ax_identifier \"command-palette-input\" \"absent\""))
