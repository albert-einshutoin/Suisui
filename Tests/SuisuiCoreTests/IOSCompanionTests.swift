...
final class IOSCompanionTests: XCTestCase {
    func testMVPConfigurationCoversRequiredMobileSurfacesAndInputs() {
        ...
        XCTAssertEqual(
            ...
            [.textConversation, .voiceInput, .shortcutsCreateTask, .shortcutsAskSuisui, .shareSheetCapture]
        )
        XCTAssertEqual(
    ...
    func testPackageManifestDeclaresIOSSupportTargetAndBuildGate() throws {
        ...
        let source = try String(contentsOfFile: "Sources/SuisuiiOS/SuisuiiOSCompanion.swift")
        XCTAssertTrue(package.contains(".iOS(.v17)"))
        XCTAssertTrue(package.contains("name: \"SuisuiiOS\""))
        XCTAssertTrue(package.contains("targets: [\"SuisuiiOS\"]"))
        XCTAssertTrue(package.contains("path: \"Sources/SuisuiiOS\""))
        XCTAssertTrue(source.contains("public struct SuisuiiOSCompanionApp: App"))
        XCTAssertTrue(source.contains("SuisuiiOSRootView()"))
    }
}
