...
final class ExternalMCPTests: XCTestCase {
    func testClientInitializesListsAndCallsToolsWith20251125Protocol() async throws {
        ...
        let result = try await client.callTool(name: "read_status", arguments: ["project": .string("suisui")])
    ...
    func testSQLiteMCPRegistrationStoreSurvivesDatabaseReopen() throws {
        let databaseURL = try temporaryDirectory().appendingPathComponent("Suisui.sqlite")
        let registration = MCPServerRegistration(
            id: "local",
            displayName: "Local MCP",
            command: "/usr/bin/env",
    ...
    func testExternalMCPExecutionInitializesSessionBeforeToolCall() async throws {
        ...
        _ = try await executor.call(
            toolName: "read_status",
            arguments: ["project": .string("suisui")],
            context: ToolExecutionContext(
                approvalToken: ApprovalToken(id: "approved", sessionID: "session"),
                source: .developerTool
            )
