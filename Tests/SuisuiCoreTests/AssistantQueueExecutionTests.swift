...
final class AssistantQueueExecutionTests: XCTestCase {
    ...
    func testCoordinatorRunsQueuedDevelopmentPrepareWorkflowWithApprovedProjectBookmark() throws {
        ...
        let branchName = "feature/suisui-\(project.id)-\(task.id)-queue-development-branch"
        ...
        gitRunner.stub(
            arguments: ["diff", "--stat"],
            output: GitCommandOutput(standardOutput: "Sources/SuisuiApp/SuisuiApp.swift | 1 +\n", standardError: "", exitCode: 0)
        )
        let registry = try ToolRegistry(tools: [
    ...
    func testCoordinatorRunsQueuedDevelopmentRepositoryCreateWithBranchAndReceiptReferences() throws {
        ...
        let branchName = "feature/suisui-\(project.id)-\(task.id)-repository-edit"
    ...
    func testCoordinatorRunsQueuedDevelopmentPushWorkflowWithApprovedProjectBookmark() throws {
        ...
        let branchName = "feature/suisui-\(project.id)-push-queue"
    ...
    func testCoordinatorRecordsSuisuiManagedUsageLedgerEntryAfterReceiptPersistence() throws {
        let queueStore = try makeQueueStore()
        let receiptStore = VolatileExecutionReceiptStore()
        ...
        XCTAssertEqual(entry.billingMode, .suisuiManaged)
    ...
    func testCoordinatorBlocksSuisuiManagedRunWhenDailyLedgerCapWouldBeExceeded() throws {
        let queueStore = try makeQueueStore()
        let receiptStore = VolatileExecutionReceiptStore()
    ...
    func testExecutableFactoryBuildsDevelopmentPullRequestReviewGatePlanFromAutomationRequest() throws {
        ...
        XCTAssertEqual(plan.actions.first?.arguments["branchName"], .string("feature/suisui-7-merge-gate"))
    ...
    func testCoordinatorRunsQueuedDevelopmentPullRequestReviewThenMergeRequestsAndPersistsReceipts() throws {
        ...
        gitRunner.stub(
            ...
            output: GitCommandOutput(
                standardOutput: "https://github.com/albert-einshutoin/suisui.git\n",
                standardError: "",
                exitCode: 0
            )
        )
    ...
    func testCoordinatorFailsDevelopmentPullRequestAutomationWithoutApprovedBookmarkBeforeGitHubCalls() throws {
        ...
        let taskStore = SQLiteTaskStore(connection: connection)
        let workspace = temporaryDirectory()
        let project = try projectStore.create(title: "Suisui", workspacePath: workspace.path)
        let branchName = "feature/suisui-\(project.id)-merge-gate"
        let pullRequestURL = "https://github.com/albert-einshutoin/suisui/pull/116"
        let gitRunner = RecordingAssistantQueueGitRunner()
        let githubRunner = RecordingAssistantQueueGitHubRunner(outputs: [
    ...
    private func temporaryDirectory() -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("SuisuiAssistantQueueExecutionTests")
