...
final class VoiceTaskReferenceResolverTests: XCTestCase {
    ...
    func testGivenSelectedTaskAndDestinationProjectWhenMoveItThenKeepsTaskTarget() {
        ...
        for utterance in [
            ...
            "move it to the Alpha project",
            "move it into the Alpha project",
            "delete it from the Alpha project",
            "delete it from the first project",
            "move it to the second project",
            "move it to this project",
            "delete it from this project",
            "それをAlphaプロジェクトに移動して",
            "それをこのプロジェクトに移動して",
            "Alphaプロジェクトからそれを削除して",
            "Alphaプロジェクト内のそれを完了して",
            "Alphaプロジェクトにあるそれを完了して",
        ] {
            XCTAssertEqual(
                resolver.resolve(
                    request(
    ...
    func testGivenModifiedTaskAnaphorThenUsesSelectionOverModifierTitle() {
        ...
        let candidates = [
            candidate(taskID: 455, projectID: 17, title: "Selected task"),
            candidate(taskID: 456, projectID: 17, title: "Important"),
        ]

        let result = resolver.resolve(
            request(
                utterance: "delete this important task",
                selectedTask: selected,
                candidates: candidates
            )
        )
        ...
        XCTAssertEqual(result, .resolved(selected, reason: .selectedTask))
        XCTAssertNotEqual(result, .resolved(modifierTitle, reason: .uniqueCandidate))
    }
    ...
    func testGivenMixedCandidateKindsThenTaskOrdinalUsesTaskOrdering() throws {
        ...
        let result = resolver.resolve(
            request(
                utterance: "open the second task",
                ordinalReference: reference,
                candidateOrderingFingerprint: fingerprint,
                candidates: candidates
            )
