...
@testable import SuisuiGoogleCalendarRuntime
...
final class GoogleCalendarAppRuntimeTests: XCTestCase {
    ...
    func testSQLiteIdempotencyNamespaceSurvivesReopeningDatabase() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("SuisuiGoogleCalendarRuntime-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }

        let databaseURL = directory.appendingPathComponent("Suisui.sqlite")
    ...
    func testHTTPEventClientCreatesEventsInsertRequestWithOAuthTokenAndStableID() throws {
        ...
        let record = try client.createEvent(
            CalendarEventDraft(
                ...
                isAllDay: true,
                notes: "Suisui",
                idempotencyKey: "suisui\(String(repeating: "a", count: 64))"
            ),
            calendarID: "primary",
        ...
        XCTAssertEqual(body["id"] as? String, "suisui\(String(repeating: "a", count: 64))")
        XCTAssertEqual(start["date"] as? String, "2026-07-07")
        XCTAssertNil(start["dateTime"])
        XCTAssertEqual(privateProperties["suisuiIdempotencyKey"] as? String, "suisui\(String(repeating: "a", count: 64))")
    ...
    func testHTTPEventClientTreatsDuplicateStableEventIDAsIdempotentSuccess() throws {
        ...
        let record = try client.createEvent(
            CalendarEventDraft(
                ...
                endAt: "2026-07-05",
                isAllDay: true,
                idempotencyKey: "suisui\(String(repeating: "b", count: 64))"
            ),
            calendarID: "primary",
            timeZoneIdentifier: "Asia/Tokyo"
        )

        XCTAssertEqual(record.event.id, "suisui\(String(repeating: "b", count: 64))")
    ...
    func testOAuthPKCEBuildsGoogleAuthorizationURLWithoutClientSecret() throws {
        let configuration = GoogleCalendarOAuthAuthorizationConfiguration(
            clientID: "google-client-id.apps.googleusercontent.com",
            redirectURI: "suisui://oauth/google-calendar"
        )
        ...
        XCTAssertEqual(query["redirect_uri"], "suisui://oauth/google-calendar")
    ...
    func testOAuthAuthorizationRejectsUnsafeRequestConfiguration() throws {
        ...
        XCTAssertThrowsError(
            try GoogleCalendarOAuthAuthorizationService(configuration: GoogleCalendarOAuthAuthorizationConfiguration(
                clientID: "google-client-id.apps.googleusercontent.com",
                redirectURI: "suisui://oauth/google-calendar?unexpected=query"
            )).makeAuthorizationRequest(state: "calendar-state", codeVerifier: "dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk")
        ) { error in
    ...
    func testOAuthCallbackExchangesCodeAndStoresTokensInSecretStore() throws {
        ...
        let metadata = try service.completeAuthorization(
            callbackURL: URL(string: "suisui://oauth/google-calendar?state=calendar-state&code=authorization-code")!,
        ...
        XCTAssertTrue(body.contains("client_id=google-client-id.apps.googleusercontent.com"))
        XCTAssertTrue(body.contains("redirect_uri=suisui%3A%2F%2Foauth%2Fgoogle-calendar"))
    ...
    func testOAuthCallbackRejectsRedirectStateAndProviderErrorsBeforeTokenExchange() throws {
        ...
        XCTAssertThrowsError(
            try service.completeAuthorization(
                callbackURL: URL(string: "suisui://oauth/other?state=calendar-state&code=authorization-code")!,
                pendingRequest: pendingRequest
            )
        ) { error in
        ...
        XCTAssertThrowsError(
            try service.completeAuthorization(
                callbackURL: URL(string: "suisui://oauth/google-calendar?state=wrong-state&code=authorization-code")!,
                pendingRequest: pendingRequest
            )
        ) { error in
        ...
        XCTAssertThrowsError(
            try service.completeAuthorization(
                callbackURL: URL(string: "suisui://oauth/google-calendar?state=calendar-state&error=access_denied")!,
                pendingRequest: pendingRequest
            )
        ) { error in
        ...
        XCTAssertThrowsError(
            try service.completeAuthorization(
                callbackURL: URL(string: "suisui://oauth/google-calendar?state=calendar-state")!,
                pendingRequest: pendingRequest
            )
        ) { error in
    ...
    func testOAuthCallbackRejectsDuplicateSecurityParametersBeforeTokenExchange() throws {
        ...
        XCTAssertThrowsError(
            try service.completeAuthorization(
                callbackURL: URL(string: "suisui://oauth/google-calendar?state=calendar-state&state=other&code=authorization-code")!,
                pendingRequest: pendingRequest
            )
        ) { error in
