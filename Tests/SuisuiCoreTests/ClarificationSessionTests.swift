...
final class ClarificationSessionTests: XCTestCase {
    func testTaskClarificationAsksOneQuestionAtATimeAndResolvesWithTrail() throws {
        ...
        XCTAssertTrue(result.resolvedRoute.planningInput.contains("project: Suisui"))
    ...
    func testDueDateDestinationRepositoryAndDocumentSourceSlotsAreParsed() throws {
        ...
        XCTAssertEqual(repositorySession.answer("suisui repo"), .resolved)
        XCTAssertEqual(try XCTUnwrap(repositorySession.result).answers[.repository], .text("suisui repo"))
