...
final class QualitySourceContractTests: XCTestCase {
    func testPseudoVoiceOverFocusPathDocumentationAndScriptCoverTaskLifecycle() throws {
        ...
        let projectBoardView = try [
            ...
            "Sources/SuisuiApp/Views/ProjectBoardDetailViews.swift",
            "Sources/SuisuiApp/Views/ProjectBoardInspectors.swift"
        ...
        XCTAssertTrue(docs.contains("SuisuiHarnessAccessibilityAuditRunner"))
        XCTAssertTrue(docs.contains("mcp-pseudo-voiceover-focus-path"))
        XCTAssertTrue(docs.contains("requiredTaskLifecycleOperations"))
        XCTAssertTrue(docs.contains("SuisuiHarnessTaskLifecycleOperation.requiredFocusNodeIDs"))
        XCTAssertTrue(docs.contains("SuisuiHarnessScenario.requiredFocusNodeIDs(for:)"))
        XCTAssertTrue(docs.contains("SuisuiHarnessTodayCockpitOperation.requiredFocusNodeIDs"))
        XCTAssertTrue(docs.contains("SuisuiHarnessScenario.requiredTodayCockpitOperations"))
        XCTAssertTrue(docs.contains("SuisuiHarnessScenario.completeTodayCockpitOperations"))
        XCTAssertTrue(docs.contains("SuisuiHarnessScenario.missingTodayCockpitOperations()"))
        ...
        XCTAssertTrue(script.contains("swift test --filter SuisuiHarnessTests"))
    ...
    func testProductOutGapLedgerClassifiesReleaseCloseoutWork() throws {
        ...
        for issueLink in [
            "https://github.com/albert-einshutoin/suisui/issues/3",
            "https://github.com/albert-einshutoin/suisui/issues/11",
            "https://github.com/albert-einshutoin/suisui/issues/14",
            "https://github.com/albert-einshutoin/suisui/issues/23",
            "https://github.com/albert-einshutoin/suisui/issues/244",
            "https://github.com/albert-einshutoin/suisui/issues/245",
            "https://github.com/albert-einshutoin/suisui/issues/246",
            "https://github.com/albert-einshutoin/suisui/issues/12"
        ] {
    ...
    func testGoogleCalendarLiveSyncEvidenceRequiresRealOAuthAndApprovedWriteProof() throws {
        let script = try readPackageFile("script/create_google_calendar_live_evidence.sh")
        let ledger = try readPackageFile("docs/release/product-out-gap-ledger.md")
        let phase15 = try readPackageFile("tasks/Phase15-ProductOutReleaseCandidate.md")
        let runbook = try readPackageFile("docs/release/manual-unblockers.md")

        for marker in [
            "docs/release/evidence/google-calendar-live-sync.md",
            "This script records evidence from a real Google Calendar OAuth live sync pass.",
            "OAuth consent completed",
            "Writable calendar list loaded",
            "Approved sync created a Google Calendar event",
            "Rerun skipped the already-linked task",
            "OAuth tokens stayed in Keychain",
            "reject_placeholder",
            ...
            XCTAssertTrue(script.contains(marker), "Google Calendar live evidence script must include \(marker)")
        }
        ...
        for forbidden in [
            ...
            XCTAssertFalse(script.contains(forbidden), "Google Calendar live evidence must not accept raw secret input: \(forbidden)")
        }
    ...
    func testManualToAutomatedRegressionBridgeDocumentsManualGateBackstops() throws {
        ...
        for requiredMarker in [
            ...
            "Tests/SuisuiCoreTests/DocumentScopedAutomationTests.swift",
            "Tests/SuisuiCoreTests/ReleasePipelineTests.swift",
            "SuisuiHarnessScenario.requiredDocumentDeliverableKinds",
            "SuisuiHarnessDocumentAutomationRunner",
            "duplicate suggested output paths",
            "SuisuiHarnessAccessibilityAuditRunner",
    ...
    func testQualityStatusDashboardScriptAndSnapshotDocumentQualityGates() throws {
        ...
        XCTAssertTrue(script.contains("SUISUI_QUALITY_STATUS_FILE"))
