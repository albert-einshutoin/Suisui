...
final class SparkleUpdateFoundationTests: XCTestCase {
    ...
    func testReleaseBuildRequiresProductionSparkleFeedConfiguration() throws {
        ...
        XCTAssertTrue(missingFeed.output.contains("SUISUI_SPARKLE_FEED_URL is required for release builds"))

        let httpFeed = try runScript(
            "script/validate_sparkle_release_config.sh",
            environment: [
                "SUISUI_BUILD_CONFIGURATION": "release",
                "SUISUI_SPARKLE_FEED_URL": "http://updates.example.invalid/suisui/appcast.xml",
                "SUISUI_SPARKLE_PUBLIC_ED_KEY": "MCowBQYDK2VwAyEATestPublicKeyForSuisuiReleaseOnly"
            ]
        )

        XCTAssertNotEqual(httpFeed.exitCode, 0)
        XCTAssertTrue(httpFeed.output.contains("SUISUI_SPARKLE_FEED_URL must use https for release builds"))

        let placeholderFeed = try runScript(
            "script/validate_sparkle_release_config.sh",
            environment: [
                "SUISUI_BUILD_CONFIGURATION": "release",
                "SUISUI_SPARKLE_FEED_URL": "https://example.com/suisui/appcast.xml",
                "SUISUI_SPARKLE_PUBLIC_ED_KEY": "MCowBQYDK2VwAyEATestPublicKeyForSuisuiReleaseOnly"
            ]
        )

        XCTAssertNotEqual(placeholderFeed.exitCode, 0)
        XCTAssertTrue(placeholderFeed.output.contains("SUISUI_SPARKLE_FEED_URL must not use placeholder or local domains for release builds"))

        let placeholderKey = try runScript(
            "script/validate_sparkle_release_config.sh",
            environment: [
                "SUISUI_BUILD_CONFIGURATION": "release",
                "SUISUI_SPARKLE_FEED_URL": "https://updates.suisui.app/releases/appcast.xml",
                "SUISUI_SPARKLE_PUBLIC_ED_KEY": "base64-public-key-from-generate_keys"
            ]
        )

        XCTAssertNotEqual(placeholderKey.exitCode, 0)
        XCTAssertTrue(placeholderKey.output.contains("SUISUI_SPARKLE_PUBLIC_ED_KEY must not use a placeholder key for release builds"))
        ...
        let malformedKey = try runScript(
            ...
            environment: [
                "SUISUI_BUILD_CONFIGURATION": "release",
                "SUISUI_SPARKLE_FEED_URL": "https://updates.suisui.app/releases/appcast.xml",
                "SUISUI_SPARKLE_PUBLIC_ED_KEY": "generated-public-ed-key-value"
            ]
        )

        XCTAssertNotEqual(malformedKey.exitCode, 0)
        XCTAssertTrue(malformedKey.output.contains("SUISUI_SPARKLE_PUBLIC_ED_KEY must be a base64 EdDSA public key for release builds"))
    ...
    func testAppcastVerifierAcceptsSparkleGeneratedElementMetadata() throws {
        ...
        let result = try runScript(
            ...
            environment: [
                ...
                "SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX": "https://updates.suisui.app/releases"
            ]
        )
    ...
    func testReleaseAppcastVerifierRejectsSamplePlaceholderSignature() throws {
        ...
        let result = try runScript(
            ...
            environment: ["SUISUI_REQUIRE_RELEASE_APPCAST": "1"]
        )
    ...
    func testReleaseAppcastVerifierRequiresConfiguredDownloadPrefix() throws {
        ...
        XCTAssertNotEqual(result.exitCode, 0)
        XCTAssertTrue(result.output.contains("release appcast enclosure URL does not match configured SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX"))
    }
    ...
    func testReleaseAppcastGeneratorRequiresProductionHTTPSPrefix() throws {
        let missingPrefix = try runScript(
            ...
            environment: [
                "SUISUI_REQUIRE_RELEASE_APPCAST": "1",
                "SUISUI_REQUIRE_SPARKLE_TOOLS": "0"
            ]
        )

        XCTAssertNotEqual(missingPrefix.exitCode, 0)
        XCTAssertTrue(missingPrefix.output.contains("SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX is required for release appcast"))
        ...
        let httpPrefix = try runScript(
            ...
            environment: [
                "SUISUI_REQUIRE_RELEASE_APPCAST": "1",
                "SUISUI_REQUIRE_SPARKLE_TOOLS": "0",
                "SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX": "http://updates.example.invalid/suisui"
            ]
        )

        XCTAssertNotEqual(httpPrefix.exitCode, 0)
        XCTAssertTrue(httpPrefix.output.contains("SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX must use https for release appcast"))
        ...
        let examplePrefix = try runScript(
            ...
            environment: [
                "SUISUI_REQUIRE_RELEASE_APPCAST": "1",
                "SUISUI_REQUIRE_SPARKLE_TOOLS": "0",
                "SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX": "https://example.com/suisui"
            ]
        )

        XCTAssertNotEqual(examplePrefix.exitCode, 0)
        XCTAssertTrue(examplePrefix.output.contains("SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX must not use placeholder or local domains for release appcast"))
        ...
        let reservedPrefix = try runScript(
            ...
            environment: [
                "SUISUI_REQUIRE_RELEASE_APPCAST": "1",
                "SUISUI_REQUIRE_SPARKLE_TOOLS": "0",
                "SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX": "https://updates.example.invalid/suisui"
            ]
        )

        XCTAssertNotEqual(reservedPrefix.exitCode, 0)
        XCTAssertTrue(reservedPrefix.output.contains("SUISUI_SPARKLE_DOWNLOAD_URL_PREFIX must not use placeholder or local domains for release appcast"))
        ...
        XCTAssertTrue(validPrefixMissingTool.output.contains("SUISUI_REQUIRE_SPARKLE_TOOLS must be 1 for release appcast generation"))
    }
