...
final class DevelopmentAutomationRuntimeSmokeTests: XCTestCase {
    func testApprovedProjectDirectoryCanEditVerifyCommitAndPreparePullRequestBranch() throws {
        ...
        let branchName = "feature/suisui-\(project.id)-\(task.id)-runtime-smoke"
        ...
        let updateResult = try DevelopmentRepositoryFileTool(
            ...
            arguments: [
                ...
                "contents": .string("# Runtime Dev Repo\n\nUpdated by Suisui development runtime smoke.\n")
            ],
            context: context
        )
        ...
        try runGit(["remote", "add", "origin", "https://github.com/albert-einshutoin/suisui.git"], in: workspace)
        let headOID = try runGit(["rev-parse", "HEAD"], in: workspace)
            .standardOutput
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let lifecycleGitRunner = RuntimeSmokePullRequestGitRunner(
            remoteURL: "https://github.com/albert-einshutoin/suisui.git",
        ...
        XCTAssertEqual(pushResult.output["remoteRepository"], .string("albert-einshutoin/suisui"))

        let pullRequestURL = "https://github.com/albert-einshutoin/suisui/pull/9999"
        ...
        XCTAssertEqual(lifecycleGitRunner.recordedInvocations, [
            ...
            GitCommandInvocation(arguments: ["remote", "get-url", "--push", "--all", "origin"], workingDirectory: workspace.resolvingSymlinksInPath().standardizedFileURL),
            GitCommandInvocation(arguments: ["rev-parse", "HEAD"], workingDirectory: workspace.resolvingSymlinksInPath().standardizedFileURL),
            GitCommandInvocation(arguments: ["ls-remote", "https://github.com/albert-einshutoin/suisui.git", "refs/heads/\(branchName)"], workingDirectory: workspace.resolvingSymlinksInPath().standardizedFileURL),
    ...
    private func temporaryDirectory() -> URL {
        let parent = ProcessInfo.processInfo.environment["SUISUI_RUNTIME_DEVELOPMENT_PR_WORKSPACE_ROOT"]
            .flatMap { $0.isEmpty ? nil : URL(fileURLWithPath: $0, isDirectory: true) }
            ?? FileManager.default.temporaryDirectory
        let url = parent
            .appendingPathComponent("SuisuiDevelopmentAutomationRuntimeSmokeTests")
    ...
    private func cleanupTemporaryDirectory(_ workspace: URL) {
        guard ProcessInfo.processInfo.environment["SUISUI_RUNTIME_DEVELOPMENT_PR_KEEP_WORKSPACE"] != "1" else {
            return
        }
    ...
    private func seedGitRepository(at workspace: URL) throws {
        ...
        try runGit(["config", "user.name", "Suisui Runtime Smoke"], in: workspace)
...
private final class RuntimeSmokeGitHubRunner: GitHubCLICommandRunner, @unchecked Sendable {
    ...
    private var pullRequestStatusJSON: String {
        """
        {
          ...
          "headRepository": {
            "name": "suisui",
            "nameWithOwner": "albert-einshutoin/suisui"
          },
          "headRepositoryOwner": {
