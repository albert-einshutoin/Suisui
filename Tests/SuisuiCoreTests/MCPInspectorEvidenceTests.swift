...
final class MCPInspectorEvidenceTests: XCTestCase {
    ...
    func testReleaseReadinessAgreesWithGeneratedInspectorEvidenceSourceCommit() throws {
        ...
        let complianceMismatch = result.output.contains(
            "MCP compliance verifier output source commit does not match current MCP source commit"
        )
        let evidenceMismatch = result.output.contains(
            "MCP Inspector evidence source commit does not match current MCP source commit"
        )
        // Stale tracked MCP evidence is a release blocker after an unrelated
        // Composition/Settings change. Keep the report fail-closed without
        // pretending that evidence was regenerated by this test.
        if complianceMismatch || evidenceMismatch {
            XCTAssertTrue(result.output.contains("BLOCKER:"), result.output)
        } else {
            XCTAssertFalse(complianceMismatch, result.output)
            XCTAssertFalse(evidenceMismatch, result.output)
        }
    }
