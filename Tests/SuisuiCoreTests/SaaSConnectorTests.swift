...
final class SaaSConnectorTests: XCTestCase {
    ...
    func testGoogleCalendarHTTPClientSendsStableEventIDAndPrivateSuisuiIdentity() throws {
        let httpClient = RecordingSynchronousHTTPDataClient(
            responseBody: #"{"id":"suisuip42t1"}"#.data(using: .utf8)!,
            statusCode: 200
        )
        let client = GoogleCalendarHTTPClient(
    ...
    func testGoogleCalendarConnectorEventSinkBridgesApprovedConnectorWrites() throws {
        ...
        let draft = CalendarEventDraft(
            ...
            isAllDay: true,
            notes: "Suisui",
            idempotencyKey: "suisui\(String(repeating: "c", count: 64))"
        )
