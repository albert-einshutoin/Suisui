...
final class DevelopmentPRWorkflowTests: XCTestCase {
    func testDeterministicBranchNameFallsBackWhenTitleContainsSecretOrLocalPath() {
        ...
        XCTAssertEqual(branchName, "feature/suisui-7-42-task")
    ...
    func testPreparePullRequestWorkflowFailsClosedWithoutApprovedProjectWorkspace() throws {
        let stores = try makeStores()
        let project = try stores.projects.create(title: "Suisui")
        let tool = DevelopmentPRWorkflowTool(
            projectStore: stores.projects,
            taskStore: stores.tasks,
    ...
    func testPreparePullRequestWorkflowCreatesDeterministicBranchInsideApprovedWorkspace() throws {
        ...
        let branchName = "feature/suisui-\(project.id)-\(task.id)-fix-calendar-sync"
        let runner = RecordingDevelopmentGitRunner()
        runner.stub(
    ...
    func testDevelopmentGitPolicyRejectsDestructiveAndExternalWriteCommands() {
        XCTAssertTrue(DevelopmentGitCommandPolicy.isAllowed(arguments: ["switch", "-c", "feature/suisui-1-task"]))
    ...
    func testPushBranchRequiresApprovedBookmarkBeforeExternalGitWrite() throws {
        ...
        let workspace = temporaryDirectory()
        let project = try stores.projects.create(title: "Suisui", workspacePath: workspace.path)
        let branchName = "feature/suisui-\(project.id)-publish-gate"
    ...
    func testPushBranchRejectsMultiplePushRepositoriesBeforeRunningExternalWrite() throws {
        let stores = try makeStores()
        let workspace = temporaryDirectory()
        let project = try createPublishProject(stores: stores, workspace: workspace)
        let branchName = "feature/suisui-\(project.id)-publish-gate"
    ...
    func testPushBranchRejectsDirtyOrWrongBranchBeforeRunningExternalWrite() throws {
        ...
        runner.stub(
            ...
            output: GitCommandOutput(
                standardOutput: """
                ## feature/other
                 M Sources/SuisuiCore/App.swift
                """,
                standardError: "",
                exitCode: 0
            )
        ...
        let result = try tool.execute(
            arguments: [
                "projectId": .number(Double(project.id)),
                "branchName": .string("feature/suisui-\(project.id)-publish-gate"),
                "expectedHeadOID": .string(headOID)
            ],
            context: approvedContext()
        )
        ...
        XCTAssertEqual(result.status, .failed)
        XCTAssertEqual(result.output["branchName"], .string("feature/suisui-\(project.id)-publish-gate"))
    ...
    func testCreatePullRequestRequiresSeparateApprovalAndBodyFileCommand() throws {
        ...
        let githubRunner = RecordingGitHubCLICommandRunner(
            output: GitHubCLICommandOutput(
                standardOutput: "https://github.com/albert-einshutoin/suisui/pull/106\n",
                standardError: "",
                exitCode: 0
            )
        )
        ...
        XCTAssertEqual(result.output["pullRequestURL"], .string("https://github.com/albert-einshutoin/suisui/pull/106"))
        XCTAssertEqual(result.output["remoteRepository"], .string("albert-einshutoin/suisui"))
        ...
        XCTAssertEqual(gitRunner.recordedInvocations, [
            ...
            GitCommandInvocation(arguments: ["remote", "get-url", "--push", "--all", "origin"], workingDirectory: workspace.standardizedFileURL.resolvingSymlinksInPath()),
            GitCommandInvocation(arguments: ["rev-parse", "HEAD"], workingDirectory: workspace.standardizedFileURL.resolvingSymlinksInPath()),
            GitCommandInvocation(arguments: ["ls-remote", "https://github.com/albert-einshutoin/suisui.git", "refs/heads/\(branchName)"], workingDirectory: workspace.standardizedFileURL.resolvingSymlinksInPath())
        ])
    }
    ...
    func testCreatePullRequestReturnsFailedResultWhenRemoteHeadIsMissing() throws {
        ...
        gitRunner.stub(
            arguments: ["ls-remote", "https://github.com/albert-einshutoin/suisui.git", "refs/heads/\(branchName)"],
            output: GitCommandOutput(standardOutput: "", standardError: "", exitCode: 0)
        )
        let githubRunner = RecordingGitHubCLICommandRunner()
        let tool = DevelopmentPullRequestCreationTool(
    ...
    func testCreatePullRequestRejectsMalformedReturnedURLBeforeReceiptOutput() throws {
        ...
        let githubRunner = RecordingGitHubCLICommandRunner(
            output: GitHubCLICommandOutput(
                standardOutput: "https://user:token@github.com/albert-einshutoin/suisui/pull/106\n",
                standardError: "",
                exitCode: 0
            )
        )
        ...
        XCTAssertThrowsError(
            try tool.execute(
                arguments: [
                    ...
                    "baseBranch": .string("main"),
                    "title": .string("Add publish gate"),
                    "body": .string("See /Volumes/Satechi/Developer/suisui/Sources/SuisuiCore/App.swift"),
                    "expectedHeadOID": .string("0123456789abcdef0123456789abcdef01234567")
                ],
                context: approvedContext()
            )
    ...
    func testPullRequestReviewGateReportsBlockedReasonsWithoutMergeCommand() throws {
        ...
        let workspace = temporaryDirectory()
        let project = try createPublishProject(stores: stores, workspace: workspace)
        let branchName = "feature/suisui-\(project.id)-merge-gate"
    ...
    func testPullRequestReviewGateRequiresApprovalBeforeExternalReads() throws {
        ...
        XCTAssertThrowsError(
            try tool.execute(
                arguments: [
                    "projectId": .number(Double(project.id)),
                    "pullRequestURL": .string("https://github.com/albert-einshutoin/suisui/pull/116"),
                    "branchName": .string("feature/suisui-\(project.id)-merge-gate"),
                    "baseBranch": .string("feature/phase14-product-completion")
                ],
                context: ToolExecutionContext(source: .reviewUI)
            )
        ...
        XCTAssertThrowsError(
            ...
            XCTAssertEqual(
                ...
                .executionFailed(
                    ...
                    "Pull request repository albert-einshutoin/suisui does not match origin repository other-org/other-repo."
                )
            )
        }
        XCTAssertEqual(githubRunner.recordedInvocations, [])
    ...
    func testPullRequestReviewGateBlocksForkHeadRepositoryAndMissingChecks() throws {
        ...
        XCTAssertTrue(result.summary.contains("head repository is outside-contributor/suisui-fork"))
    }
    ...
    func testDevelopmentPublishPoliciesRejectUnsafeCommands() {
        ...
        XCTAssertTrue(DevelopmentPublishGitCommandPolicy.isAllowed(arguments: ["ls-remote", "origin", "refs/heads/feature/suisui-1-task"]))
        XCTAssertTrue(DevelopmentPublishGitCommandPolicy.isAllowed(arguments: ["ls-remote", "git@github.com:albert-einshutoin/suisui.git", "refs/heads/feature/suisui-1-task"]))
        XCTAssertFalse(DevelopmentPublishGitCommandPolicy.isAllowed(arguments: ["ls-remote", "ssh://git@example.com/albert-einshutoin/suisui.git", "refs/heads/feature/suisui-1-task"]))
        XCTAssertFalse(DevelopmentPublishGitCommandPolicy.isAllowed(arguments: ["ls-remote", "origin", "refs/heads/main"]))
        let headOID = String(repeating: "a", count: 40)
        XCTAssertTrue(DevelopmentPublishGitCommandPolicy.isAllowed(arguments: ["push", "-u", "origin", "\(headOID):refs/heads/feature/suisui-1-task"]))
        XCTAssertFalse(DevelopmentPublishGitCommandPolicy.isAllowed(arguments: ["push", "-u", "origin", "feature/suisui-1-task"]))
        XCTAssertFalse(DevelopmentPublishGitCommandPolicy.isAllowed(arguments: ["push"]))
        XCTAssertFalse(DevelopmentPublishGitCommandPolicy.isAllowed(arguments: ["push", "--force", "origin", "feature/suisui-1-task"]))
        ...
        let bodyFile = FileManager.default.temporaryDirectory.appendingPathComponent("suisui-pr-body-test.md").path
        XCTAssertTrue(DevelopmentGitHubPRCommandPolicy.isAllowed(arguments: [
            "pr", "create",
            "--repo", "albert-einshutoin/suisui",
            "--base", "main",
            "--head", "feature/suisui-1-task",
        ...
        XCTAssertTrue(DevelopmentGitHubPRCommandPolicy.isAllowed(arguments: [
            "pr", "view", "https://github.com/albert-einshutoin/suisui/pull/116",
            "--json", DevelopmentGitHubPRCommandPolicy.statusJSONFields
        ]))
        XCTAssertTrue(DevelopmentGitHubPRCommandPolicy.isAllowed(arguments: [
            "api", "graphql",
        ...
        XCTAssertTrue(DevelopmentGitHubPRCommandPolicy.isAllowed(arguments: [
            "pr", "merge", "https://github.com/albert-einshutoin/suisui/pull/116",
            "--merge", "--delete-branch",
        ...
        XCTAssertFalse(DevelopmentGitHubPRCommandPolicy.isAllowed(arguments: [
            "pr", "view", "https://user:token@github.com/albert-einshutoin/suisui/pull/116",
            "--json", DevelopmentGitHubPRCommandPolicy.statusJSONFields
        ]))
        XCTAssertFalse(DevelopmentGitHubPRCommandPolicy.isAllowed(arguments: [
            "pr", "view", "https://github.com/albert-einshutoin/suisui/pull/116?merge=1",
            "--json", DevelopmentGitHubPRCommandPolicy.statusJSONFields
        ]))
        XCTAssertFalse(DevelopmentGitHubPRCommandPolicy.isAllowed(arguments: [
            "pr", "view", "https://github.com/albert-einshutoin/suisui/pull/not-a-number",
            "--json", DevelopmentGitHubPRCommandPolicy.statusJSONFields
        ]))
        XCTAssertFalse(DevelopmentGitHubPRCommandPolicy.isAllowed(arguments: [
            "pr", "view", "https://github.com/albert-einshutoin//suisui/pull/116",
    ...
    func testPullRequestReviewGateRejectsMalformedOriginRemoteBeforeGitHubRead() throws {
        ...
        let project = try createPublishProject(stores: stores, workspace: workspace)
        let gitRunner = RecordingDevelopmentGitRunner()
        stubOrigin(gitRunner, remoteURL: "https://github.com/albert-einshutoin//suisui.git")
        let githubRunner = RecordingGitHubCLICommandRunner()
        let tool = DevelopmentPullRequestReviewGateTool(
            projectStore: stores.projects,
            gitRunner: gitRunner,
    ...
    func testRuntimeSmokeGitHubRunnerBlocksUnexpectedCommandsBeforeLiveCLI() throws {
        ...
        let pullRequestURL = "https://github.com/albert-einshutoin/suisui/pull/116"
        let branchName = "feature/suisui-7-review-gate"
        ...
        try withRuntimeDevelopmentGitHubSmokeEnvironment(
            ...
            let differentURLOutput = try runner.runGitHub(
                arguments: [
                    "pr", "view", "https://github.com/albert-einshutoin/suisui/pull/99999",
                    "--json", DevelopmentGitHubPRCommandPolicy.statusJSONFields
                ],
                workingDirectory: workspace
            )
        ...
        XCTAssertTrue(blockedLogText.contains("gh pr view https://github.com/albert-einshutoin/suisui/pull/99999"))
    ...
    func testPreparePullRequestWorkflowRejectsSymlinkWorkspace() throws {
        ...
        let project = try stores.projects.create(title: "Suisui", workspacePath: symlink.path)
        let runner = RecordingDevelopmentGitRunner()
        let tool = DevelopmentPRWorkflowTool(
            projectStore: stores.projects,
            taskStore: stores.tasks,
    ...
    private func temporaryDirectory() -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("SuisuiDevelopmentPRWorkflowTests")
    ...
    private func withRuntimeDevelopmentGitHubSmokeEnvironment(
        ...
        let values = [
            ...
            "SUISUI_RUNTIME_DEVELOPMENT_PR_EXPECTED_BRANCH": expectedBranch,
            "SUISUI_RUNTIME_DEVELOPMENT_PR_EXPECTED_HEAD": expectedHead,
            "SUISUI_RUNTIME_DEVELOPMENT_PR_EXPECTED_BASE": expectedBase,
            "SUISUI_RUNTIME_DEVELOPMENT_PR_CREATE_URL": pullRequestURL,
            "SUISUI_RUNTIME_DEVELOPMENT_PR_CREATE_LOG": createLog.path,
            "SUISUI_RUNTIME_DEVELOPMENT_PR_REVIEW_LOG": reviewLog.path,
            "SUISUI_RUNTIME_DEVELOPMENT_PR_MERGE_LOG": mergeLog.path,
            "SUISUI_RUNTIME_DEVELOPMENT_PR_BLOCKED_EXTERNAL_WRITE_LOG": blockedLog.path
        ]
    ...
    private func stubOrigin(
        ...
        remoteURL: String = "https://github.com/albert-einshutoin/suisui.git"
    ) {
        runner.stub(
