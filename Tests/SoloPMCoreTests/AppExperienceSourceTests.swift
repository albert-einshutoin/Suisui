...
final class AppExperienceSourceTests: XCTestCase {
    ...
    func testUIScreenshotCaptureVerifiesTargetDestinationBeforeScreenshot() throws {
        ...
        XCTAssertTrue(script.contains("AX marker scans are bounded to one traversal per visible window"))
        XCTAssertTrue(script.contains("on scanElementTree(uiElement, identifierNeedle, textNeedle)"))
        XCTAssertTrue(script.contains("if itemIdentifier contains identifierNeedle then set foundIdentifier to true"))
        XCTAssertTrue(script.contains("if foundIdentifier and foundText then return {true, true}"))
        XCTAssertFalse(script.contains("if not foundIdentifier and my elementTreeContains(currentWindow, identifierNeedle) then set foundIdentifier to true"))
        XCTAssertFalse(script.contains("if not foundText and my elementTreeContains(currentWindow, textNeedle) then set foundText to true"))
