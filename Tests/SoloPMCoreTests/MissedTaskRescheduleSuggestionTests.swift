...
final class MissedTaskRescheduleSuggestionTests: XCTestCase {
    ...
    func testFridayRunSkipsWeekendAndTargetsMondayWithShiftReason() throws {
        ...
        XCTAssertEqual(result.enqueuedItemIDs, ["solopm-reschedule-7-2026-07-10"])
        let saved = try queueStore.get(id: "solopm-reschedule-7-2026-07-10")
        guard case let .actionPlan(plan) = saved.payload else {
            XCTFail("Expected an action plan payload.")
            return
        }
    ...
    func testSaturdayRunAlsoTargetsMonday() throws {
        ...
        XCTAssertEqual(result.enqueuedItemIDs, ["solopm-reschedule-8-2026-07-11"])
        let saved = try queueStore.get(id: "solopm-reschedule-8-2026-07-11")
        guard case let .actionPlan(plan) = saved.payload else {
            XCTFail("Expected an action plan payload.")
            return
        }
    ...
    func testWeekendAvoidanceDisabledKeepsTomorrowEvenOnSaturday() throws {
        ...
        XCTAssertEqual(result.enqueuedItemIDs, ["solopm-reschedule-9-2026-07-10"])
        let saved = try queueStore.get(id: "solopm-reschedule-9-2026-07-10")
        guard case let .actionPlan(plan) = saved.payload else {
            XCTFail("Expected an action plan payload.")
            return
        }
    ...
    func testWeekdayRunKeepsTomorrowReasonWording() throws {
        ...
        let saved = try queueStore.get(id: "solopm-reschedule-10-2026-07-07")
        XCTAssertEqual(
    ...
    func testOpenRescheduleSuggestionIDsIgnoreNonRescheduleAndNonWaitingItems() throws {
        ...
        XCTAssertFalse(
            fixture.viewModel.openRescheduleSuggestionIDs.contains("solopm-reschedule-3-2026-07-06")
        )
    }
}
