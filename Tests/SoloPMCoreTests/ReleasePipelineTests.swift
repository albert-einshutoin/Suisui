...
final class ReleasePipelineTests: XCTestCase {
    ...
    func testReleaseActionSummaryReportsManualHelperFreshnessForCurrentCommit() throws {
        ...
        XCTAssertTrue(actions.contains("- [x] VoiceOver worksheet is generated for current source commit: `.tmp/voiceover-review/voiceover-worksheet.md`"))
        ...
        XCTAssertTrue(actions.contains("Next: complete generated `.tmp/voiceover-review/create-evidence-command.sh`, run its validate-only path first, then rerun readiness."))
        XCTAssertTrue(actions.contains("- [ ] Competitor hands-on pass clears up to"))
        XCTAssertTrue(actions.contains("Next: fill generated `.tmp/competitor-hands-on/create-evidence-command.sh` after the 2-4h pass, run its validate-only path first, then rerun readiness."))
        ...
        XCTAssertTrue(actions.contains("- [!] `.tmp/voiceover-review/accessibility-voiceover-pending-oldcafe.md` is ignored because the current source commit is `\(commit)`."))
        ...
        XCTAssertTrue(actions.contains("- [!] `.tmp/competitor-hands-on/evidence.md` is ignored because the current source commit is `\(commit)`."))
    ...
    func testReleaseActionSummaryTellsOperatorsToRegenerateStaleManualHelpers() throws {
        ...
        XCTAssertTrue(actions.contains("- [ ] VoiceOver worksheet missing for current source commit: `.tmp/voiceover-review/voiceover-worksheet.md`"))
    ...
    func testReleaseActionSummaryRejectsCommandHelpersThatOnlyMentionCurrentCommitInComments() throws {
        ...
        XCTAssertTrue(actions.contains("- [ ] VoiceOver evidence command is stale or not pinned to current source commit `\(commit)`: `.tmp/voiceover-review/create-evidence-command.sh`"))
        XCTAssertTrue(actions.contains("- [ ] Competitor evidence command is stale or not pinned to current source commit `\(commit)`: `.tmp/competitor-hands-on/create-evidence-command.sh`"))
        XCTAssertTrue(actions.contains("- [ ] Release evidence command is stale or not pinned to current source commit `\(commit)`: `.tmp/release-machine/create-release-evidence-command.sh`"))
        XCTAssertTrue(actions.contains("NEXT: regenerate manual review helpers for current source commit before running any passed-evidence command."))
    }
    ...
    func testReleaseReadinessReportCanUseAutomatedPreflightEvidenceInsteadOfRerunningLocalProofGates() throws {
        ...
        XCTAssertTrue(actionSummary.contains("The generated competitor hands-on evidence command also refuses to run until `.tmp/competitor-hands-on/hands-on-worksheet.md` is `Status: completed`, pinned to the same source commit, free of pending/unchecked/template markers, and filled."))
    ...
    func testReleaseReadinessReportCanWriteOperatorActionSummaryWithoutPassingManualGates() throws {
        ...
        XCTAssertTrue(script.contains("complete generated `.tmp/voiceover-review/create-evidence-command.sh`, run its validate-only path first, then rerun readiness."))
        ...
        XCTAssertTrue(script.contains("fill generated `.tmp/competitor-hands-on/create-evidence-command.sh` after the 2-4h pass, run its validate-only path first, then rerun readiness."))
        ...
        XCTAssertTrue(script.contains("complete generated `.tmp/release-machine/create-release-evidence-command.sh` after signing/notarization/Sparkle/Gatekeeper checks, run its validate-only path first, then rerun readiness."))
        XCTAssertTrue(script.contains("Login Item manual check is part of Phase checklist routing. Next: use the signed release app, complete \\`--login-item-toggle\\` in \\`.tmp/release-machine/create-release-evidence-command.sh\\`, then rerun readiness."))
        ...
        XCTAssertTrue(script.contains("The generated competitor hands-on evidence command also refuses to run until \\`.tmp/competitor-hands-on/hands-on-worksheet.md\\` is \\`Status: completed\\`, pinned to the same source commit, free of pending/unchecked/template markers, and filled."))
        ...
        XCTAssertTrue(checklist.contains("`./script/prepare_release_manual_helpers.sh --prune-stale` removes ignored old pending previews and legacy default preview files after the current helpers are regenerated"))
        ...
        XCTAssertTrue(checklist.contains("The generated release-machine command refuses to run until its worksheet is `Status: completed`, source-pinned, filled, and free of pending/unchecked/template markers."))
        ...
        XCTAssertTrue(phase.contains("[x] `script/prepare_release_manual_helpers.sh` は current source commit の VoiceOver pending preview / launch env / worksheet / command、competitor pending evidence、competitor benchmark pending worksheet、competitor worksheet / command、release-machine worksheet / command を一括再生成し、passed evidence を書かない。"))
        ...
        XCTAssertTrue(phase.contains("[x] `script/prepare_release_manual_helpers.sh --prune-stale` は current source commit のhelper再生成後、古いpending previewとlegacy default previewを削除し、passed evidenceを書かない。"))
