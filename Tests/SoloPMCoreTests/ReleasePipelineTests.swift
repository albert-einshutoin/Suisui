...
final class ReleasePipelineTests: XCTestCase {
    ...
    func testVoiceOverEvidenceGeneratorWritesPendingAndPassedEvidence() throws {
        ...
        XCTAssertTrue(passedEvidence.contains("- Runtime AX smoke: OK: runtime AX smoke visible, windows=1, window=1 name=SoloPM, buttons=28, textFields=1, staticTexts=24, unlabeledButtons=0, genericButtons=0, crudSignals=8/8"))
    ...
    func testAccessibilityPreflightChecksSourceAnchorsAndDocumentsRuntimeBoundary() throws {
        ...
        XCTAssertTrue(phase.contains("[x] VoiceOver passed evidence は同じrelease候補で実行したruntime AX smoke OK行、`unlabeledButtons=0`、`genericButtons=0`、`crudSignals=8/8` を含まない場合release readyにしない。"))
    ...
    func testRuntimeAccessibleCRUDSmokeScriptLaunchesIsolatedAppAndVerifiesSQLiteMutations() throws {
        ...
        XCTAssertTrue(script.contains("waitForTextFieldContaining \"Enter the task name\""))
        XCTAssertTrue(script.contains("setTextFieldContaining \"Enter the task name\" \"AX Runtime CRUD Task\""))
        XCTAssertTrue(script.contains("pressButtonContaining \"Creates the task in the local SoloPM database.\""))
        XCTAssertTrue(script.contains("pressButtonContaining \"Opens task details in the inspector\""))
        XCTAssertTrue(script.contains("setTextFieldContaining \"AX Runtime CRUD Task\" \"AX Runtime CRUD Task Updated\""))
        ...
        XCTAssertTrue(script.contains("pressButtonContaining \"Deletes the selected task after confirmation.\""))
        XCTAssertTrue(script.contains("pressConfirmationButtonContaining \"Delete Task\" \"Deletes the selected task after confirmation.\""))
        XCTAssertTrue(script.contains("setTextFieldContaining \"Enter the task name\" \"AX Runtime Cascade Task\""))
        ...
        XCTAssertTrue(script.contains("verify_single_value \"created task\""))
        ...
        XCTAssertTrue(script.contains("verify_single_value \"deleted task\""))
        XCTAssertTrue(script.contains("verify_single_value \"created cascade task\""))
    ...
    func testReleaseReadinessReportFailsWhenUIScreenshotEvidenceIsBlank() throws {
        ...
        for screenshotFilename in ["project-board-light.png", "project-board-dark.png", "project-board-system.png", "settings-appearance-light.png", "settings-appearance-dark.png", "settings-mcp-light.png", "settings-mcp-dark.png"] {
            try writeSolidPNG(
