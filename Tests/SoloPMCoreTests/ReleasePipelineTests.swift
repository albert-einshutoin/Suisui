...
final class ReleasePipelineTests: XCTestCase {
    ...
    func testRuntimeAccessibleCRUDSmokeScriptLaunchesIsolatedAppAndVerifiesSQLiteMutations() throws {
        ...
        XCTAssertTrue(script.contains("HOME=\"$runtime_home\""))
        ...
        XCTAssertTrue(script.contains("DESTRUCTIVE_POSTCONDITION_TIMEOUT_SECONDS=\"${SOLOPM_RUNTIME_ACCESSIBLE_CRUD_DESTRUCTIVE_POSTCONDITION_TIMEOUT_SECONDS:-10}\""))
        XCTAssertTrue(script.contains("FORM_POSTCONDITION_TIMEOUT_SECONDS=\"${SOLOPM_RUNTIME_ACCESSIBLE_CRUD_FORM_POSTCONDITION_TIMEOUT_SECONDS:-10}\""))
        ...
        XCTAssertFalse(script.contains("key code 51"))
        XCTAssertFalse(script.contains("key code 48"))
        ...
        XCTAssertTrue(script.contains("local postcondition_deadline=$((SECONDS + FORM_POSTCONDITION_TIMEOUT_SECONDS))"))
        XCTAssertTrue(script.contains("if [[ \"$SECONDS\" -ge \"$postcondition_deadline\" ]]; then\n        break\n      fi\n      activate_app"))
        ...
        XCTAssertTrue(script.contains("local postcondition_deadline=$((SECONDS + DESTRUCTIVE_POSTCONDITION_TIMEOUT_SECONDS))"))
    ...
    func testRuntimeDevelopmentPRSmokeScriptRunsApprovedDirectoryFixtureFlow() throws {
        ...
        XCTAssertTrue(projectBoardViewSource.contains("@State private var repositoryEditRelativePath"))
        XCTAssertTrue(projectBoardViewSource.contains("@State private var repositoryEditContents"))
        XCTAssertTrue(projectBoardViewSource.contains("@State private var commitRelativePaths"))
        XCTAssertTrue(projectBoardViewSource.contains("@State private var commitMessage"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-edit-path"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-edit-contents"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-edit-preview"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-edit-queue"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-verification-queue"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-commit-paths"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-commit-message"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-commit-queue"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-push-queue"))
        ...
        XCTAssertTrue(projectBoardViewSource.contains("enqueueDevelopmentPushReview"))
        XCTAssertTrue(projectBoardViewSource.contains("project-development-automation-pr-merge-queue"))
        XCTAssertTrue(projectBoardViewSource.contains("canQueuePullRequestMergeGate"))
    ...
    func testRuntimeInboxTriageSmokeScriptVerifiesAllClassificationActionsAndUndo() throws {
        ...
        XCTAssertTrue(textInputHelper.contains("NSWorkspace.shared.frontmostApplication?.processIdentifier == pid"))
        XCTAssertTrue(textInputHelper.contains("post(tap: .cghidEventTap)"))
        XCTAssertTrue(textInputHelper.contains("NSPasteboard.general"))
        XCTAssertTrue(textInputHelper.contains("savedPasteboardItems"))
        XCTAssertTrue(textInputHelper.contains("restorePasteboard"))
        XCTAssertTrue(textInputHelper.contains("keyCode: 9"))
        XCTAssertFalse(textInputHelper.contains("keyboardSetUnicodeString"))
    ...
    func testRuntimeTodayCompleteSmokeScriptVerifiesVisibleRowCompletionPersistsToSQLite() throws {
        ...
        XCTAssertTrue(script.contains("pressButtonContainingBounded \"workflow-task-completion-$today_task_id\""))
    ...
    func testRuntimeTodayProductionRouteSmokeScriptDefinesIsolatedMatrixAndConvergenceGate() throws {
        ...
        XCTAssertTrue(script.contains("expected_today_label_for"))
        XCTAssertTrue(script.contains("english) printf '%s' \"Today\""))
        XCTAssertTrue(script.contains("japanese) printf '%s' \"今日\""))
        ...
        XCTAssertTrue(script.contains("CPU_CONVERGENCE_TIMEOUT_SECONDS=\"${SOLOPM_RUNTIME_TODAY_CPU_CONVERGENCE_TIMEOUT_SECONDS:-10}\""))
        XCTAssertTrue(script.contains("case_deadline=$((SECONDS + CPU_CONVERGENCE_TIMEOUT_SECONDS))"))
        ...
        XCTAssertTrue(script.contains("launch_route_process_and_window \"$route_artifact_dir/window-attempt-1.err\""))
        XCTAssertTrue(script.contains("launch_route_process_and_window \"$route_artifact_dir/window-attempt-2.err\""))
        XCTAssertTrue(script.contains("$route_failure_category\" != \"window"))
        XCTAssertTrue(script.contains("retrying production route after owned window publication timeout"))
        ...
        XCTAssertTrue(script.contains("diagnostic-unavailable"))
        XCTAssertTrue(script.contains("date '+%Y-%m-%dT12:00:00%z'"))
        XCTAssertFalse(script.contains("date -u '+%Y-%m-%dT12:00:00Z'"))
        XCTAssertTrue(script.contains("small_fixture_today_due_at"))
        XCTAssertFalse(script.contains("VALUES (1, 'fixture-today-1'"))
        XCTAssertTrue(script.contains("verify_small_fixture_today_data"))
        XCTAssertTrue(script.contains("source_command='runtime-today-production-route'"))
        XCTAssertTrue(script.contains("failure_reason"))
        XCTAssertTrue(script.contains("fail_route \"product-marker\" \"cpu-convergence-timeout\""))
        ...
        XCTAssertFalse(script.contains("route_failure_category=\"fixture\""))
        XCTAssertTrue(script.contains("ax_classify_window_failure \"$window_diagnostic\" \"$app_pid\""))
    ...
    func testRuntimeTodayProductionRouteSmokeUsesPIDScopedAXAndOptInRuntimeGate() throws {
        ...
        let axHelpers = try readPackageFile("script/ui_accessibility_smoke_helpers.sh")
        let ci = try readPackageFile("scripts/ci.sh")

        XCTAssertTrue(script.contains("capture_sanitized_windows()"))
        XCTAssertTrue(script.contains("unix id is appPID"))
        XCTAssertTrue(script.contains("ax_wait_for_ax_identifier \"$APP_NAME\" \"$marker\" 1 \"$ROOT_DIR\" \"$probe_file\" \"$required_text\" \"$app_pid\""))
        XCTAssertTrue(axHelpers.contains("local text_marker=\"${6:-}\""))
        XCTAssertTrue(axHelpers.contains("local app_pid=\"${7:-}\""))
        XCTAssertTrue(axHelpers.contains("local ax_arguments=(\"$app_name\" \"$identifier\" \"$text_marker\")"))
        XCTAssertTrue(axHelpers.contains("if [[ -n \"$app_pid\" ]]; then"))
        XCTAssertTrue(axHelpers.contains("\"${ax_arguments[@]}\""))
    ...
    func testRuntimeSettingsSaveSmokeScriptPersistsNonSecretSettingsToIsolatedUserDefaults() throws {
        ...
        XCTAssertTrue(script.contains("SOLOPM_SETTINGS_SMOKE_GOOGLE_CALENDAR_ID=\"$runtime_google_calendar_id\""))
        ...
        let pressControlStart = try XCTUnwrap(script.range(of: "pressControlContaining() {"))
        let setTextFieldStart = try XCTUnwrap(script.range(of: "setTextFieldContaining() {"))
        let pressControlSource = String(script[pressControlStart.lowerBound..<setTextFieldStart.lowerBound])
        XCTAssertTrue(pressControlSource.contains("local pressControlAttemptSeconds=\"${SOLOPM_RUNTIME_SETTINGS_AX_PRESS_ATTEMPT_SECONDS:-30}\""))
        XCTAssertTrue(pressControlSource.contains("local osascript_pid=$!"))
        XCTAssertTrue(pressControlSource.contains("set itemActions to name of actions of axItem"))
        XCTAssertTrue(pressControlSource.contains("if itemActions contains \"AXPress\" then"))
        XCTAssertTrue(pressControlSource.contains("kill \"$osascript_pid\""))
        XCTAssertTrue(pressControlSource.contains("wait \"$osascript_pid\""))

        let waitForAXStart = try XCTUnwrap(script.range(of: "waitForAXElementContaining() {"))
        let pressControlFunctionStart = try XCTUnwrap(script.range(of: "pressControlContaining() {"))
        let waitForAXSource = String(script[waitForAXStart.lowerBound..<pressControlFunctionStart.lowerBound])
        XCTAssertTrue(waitForAXSource.contains("local axQueryAttemptSeconds=\"${SOLOPM_RUNTIME_SETTINGS_AX_QUERY_ATTEMPT_SECONDS:-30}\""))
        XCTAssertTrue(waitForAXSource.contains("local osascript_pid=$!"))
        XCTAssertTrue(waitForAXSource.contains("kill \"$osascript_pid\""))

        let enableCheckboxStart = try XCTUnwrap(script.range(of: "enableCheckboxContaining() {"))
        let setTextFieldSource = String(script[setTextFieldStart.lowerBound..<enableCheckboxStart.lowerBound])
        XCTAssertTrue(setTextFieldSource.contains("local axTextAttemptSeconds=\"${SOLOPM_RUNTIME_SETTINGS_AX_TEXT_ATTEMPT_SECONDS:-30}\""))
        XCTAssertTrue(setTextFieldSource.contains("local osascript_pid=$!"))
    ...
    func testLayoutStabilitySmokeScriptSamplesImmediateFramesAndWritesArtifacts() throws {
        ...
        XCTAssertTrue(script.contains("SOLOPM_DATABASE_PATH=\"$LAYOUT_STABILITY_DATABASE_PATH\""))
        ...
        XCTAssertTrue(frameHelper.contains("kAXPositionAttribute as CFString"))
        XCTAssertTrue(frameHelper.contains("kAXSizeAttribute as CFString"))
        XCTAssertTrue(script.contains("\"$AX_PRESS_ELEMENT_HELPER_BINARY\" \"$app_pid\" \"$destination_identifier\""))
        XCTAssertFalse(script.contains("/usr/bin/swift \"$AX_PRESS_ELEMENT_HELPER\""))
        XCTAssertTrue(pressElementHelper.contains("NSRunningApplication(processIdentifier: appPID)"))
        XCTAssertTrue(pressElementHelper.contains("AXUIElementCopyActionNames"))
        ...
        XCTAssertTrue(script.contains("assert_sidebar_destination_window_size_stable()"))
        ...
        XCTAssertTrue(script.contains("LAYOUT_STABILITY_CLIPPING_TOLERANCE_PX=\"${SOLOPM_LAYOUT_STABILITY_CLIPPING_TOLERANCE_PX:-1}\""))
        XCTAssertTrue(script.contains("-v clipTolerance=\"$LAYOUT_STABILITY_CLIPPING_TOLERANCE_PX\""))
