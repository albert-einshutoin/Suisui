...
final class ReleasePipelineTests: XCTestCase {
    ...
    func testLayoutStabilitySmokeScriptSamplesImmediateFramesAndWritesArtifacts() throws {
        ...
        XCTAssertTrue(script.contains("set axItems to entire contents of currentWindow"))
        XCTAssertTrue(script.contains("repeat with axItem in axItems"))
        XCTAssertFalse(script.contains("on collectIdentifiedElements(outputLines, uiElement)"))
        ...
        XCTAssertTrue(script.contains("# selected row before every probe keeps SwiftUI rebuilding the AX subtree.\n    ensure_project_detail_visible"))
        XCTAssertFalse(script.contains("while true; do\n    ensure_project_detail_visible\n    if collect_ax_frames"))
