...
EOF
  ...
  cursor-agent --print --mode ask --trust --model "$CURSOR_MODEL" \
    --workspace "$(pwd)" "$(cat "$prompt_file")" >"$draft_file"
}
...
Use the project sources and Cursor Agent scout notes to produce at most ${LIMIT}
...
- Each issue body must be implementation-ready for a junior engineer and make
  the following explicit: why the change is needed, the goal, implementation
  approach, cautions and trust boundaries, quality invariants, a RED/GREEN TDD
  plan, acceptance criteria, and verification commands.
...
main() {
  ...
  local tmpdir source_file draft_file final_file cursor_prompt codex_prompt
  tmpdir="$(mktemp -d)"
  source_file="${tmpdir}/sources.md"
  draft_file="${tmpdir}/cursor-draft.md"
  final_file="${tmpdir}/issues.json"
  cursor_prompt="${tmpdir}/cursor-prompt.md"
  ...
  draft_with_cursor "$source_file" "$draft_file" "$cursor_prompt"
